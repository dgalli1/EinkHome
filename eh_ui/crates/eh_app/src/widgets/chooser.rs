...
pub(crate) const SORT_KEYS: [&str; 4] = ["sort.title_az", "sort.author", "sort.series", "sort.recent"];
...
pub fn draw_chooser_sheet<B: Framebuffer>(
    ...
    let sh = super::sheet::open_sheet(surf, dirty, h, 0, h, h, (96 + n as u32 * 96 + 24).max(1), true);
    ...
    eh_render::draw_text(surf, eh_shell::bold_font(), 28.0, title, (sh.px + 24) as i32, (sh.py + 16) as i32 + 22, GRAY_BLACK, &mut g);
    ...
    eh_render::draw_text(surf, font, 24.0, &current, (sh.px + 24) as i32, (sh.py + 46) as i32 + 18, GRAY_DGRAY, &mut g);
    ...
    for (i, label) in labels.iter().enumerate() {
        ...
        surf.fill_gray(Rect { x: sh.px + 12, y: iy, w: sh.pw - 24, h: 84 }, bg);
        surf.rect_outline(Rect { x: sh.px + 12, y: iy, w: sh.pw - 24, h: 84 }, 1, GRAY_BLACK);
        eh_render::draw_text(surf, eh_shell::bold_font(), 26.0, label, (sh.px + 32) as i32, (iy + 30) as i32 + 20, fg, &mut g);
        app.chooser_rects.push(Rect { x: sh.px + 12, y: iy, w: sh.pw - 24, h: 84 });
    }
}

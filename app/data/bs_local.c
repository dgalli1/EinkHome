/* bs_local.c — part of the bookshelf app (see bs_core.h) */
...
#define BS_LOCAL_SCAN_DEPTH 8
#define BS_LOCAL_SCAN_CAP 20000
...
{
    if (res->count >= BS_LOCAL_SCAN_CAP) {
        res->truncated = 1;
        return NULL;
    }
...
{
    if (depth > BS_LOCAL_SCAN_DEPTH || res->count >= BS_LOCAL_SCAN_CAP)
        return;
    DIR *d = opendir(dir);
    if (d == NULL)
        return;
...
local_scan_walk(BsJob *job)
{
    ...
    folder_scan_collect(BS_BROWSE_ROOT, 0, res);
    if (res->truncated)
        bs_LOG("[bookshelf] local: scan cap %d reached, import truncated\n",
            BS_LOCAL_SCAN_CAP);
    job->result = res;
    job->rc = 0;
    atomic_store_explicit(&job->done, 1, memory_order_release);
...
local_file_to_book(const BsLocalFile *f, BsBook *b)
{
    ...
    char mtitle[BS_MAX_TITLE_LEN], mauthor[80];
    if (bs_store_local_meta_get(f->id, mtitle, sizeof mtitle, mauthor,
                             sizeof mauthor)) {
        if (mtitle[0] != '\0')
            snprintf(b->title, sizeof b->title, "%s", mtitle);
        if (mauthor[0] != '\0')
            snprintf(b->author, sizeof b->author, "%s", mauthor);
    } else if (bs_extract_book_meta(f->local_path, f->ext, mtitle,
        ...
        if (mauthor[0] != '\0')
            snprintf(b->author, sizeof b->author, "%s", mauthor);
        bs_store_local_meta_put(f->id, mtitle, mauthor);
    }
}
...
local_apply_slice(BsJob *job)
{
    ...
    int end = a->offset + BS_SYNC_BATCH;
    if (end > res->count)
        end = res->count;
    if (bs_store_begin() != 0) {
        ...
        bs_LOG("[bookshelf] local: store_begin failed; aborting import\n");
        local_apply_fail(a);
        return;
    }
    if (a->offset == 0)
        bs_store_delete_source(res->src);
    for (int i = a->offset; i < end; i++) {
        BsBook b;
        local_file_to_book(&res->books[i], &b);
        if (bs_store_upsert_book(&b) != 0) {
            ...
            bs_LOG("[bookshelf] local: upsert failed id=%s; "
    ...
    if (bs_g_state.sync_popup && bs_g_state.sync_stage == BS_SYNC_STAGE_SCAN) {
        bs_g_state.sync_scan = a->offset;
    ...
    bs_LOG("[bookshelf] local: imported %d books (%s) from %s\n", res->count,
        res->src, BS_BROWSE_ROOT);
    free(res->books);
    free(res);
    free(a);
    bs_sync_local_finish();
...
void
bs_local_import_scanner(void)
{
    g_local_scan_gen++;
    BsLocalApplyArg *a = malloc(sizeof *a);
    if (a == NULL) {
        bs_LOG("[bookshelf] local: import start alloc failed\n");
        bs_sync_local_finish();
        return;
    }
    a->res = NULL;
    ...
    if (bs_worker_submit(local_scan_walk, local_scan_start_apply, a) == NULL) {
        free(a);
        bs_LOG("[bookshelf] local: worker submit failed\n");
        bs_sync_local_finish();
        return;
    }
    bs_LOG("[bookshelf] local: import scan started\n");
}

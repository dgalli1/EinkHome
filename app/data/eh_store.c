/* eh_store.c - part of the bookshelf app (see eh_core.h)
 *
...
static char g_search_q_cache[EH_MAX_QUERY_LEN];
static char g_fts_query[EH_MAX_QUERY_LEN * 4 + 16];
...
static void store_path(char *out, size_t cap) {
  char dir[EH_MAX_PATH_LEN];
  eh_dirname_of(eh_g_config_path, dir, sizeof dir);
  snprintf(out, cap, "%s/%s", dir, EH_LIB_DB_FILENAME);
}
...
static void store_migrate_columns(void) {
  ...
  for (size_t i = 0; i < sizeof mig / sizeof mig[0]; i++) {
    ...
    if (sqlite3_exec(g_db, sql, NULL, NULL, NULL) != SQLITE_OK)
      eh_LOG("[bookshelf] store: migrate %s failed: %s\n", mig[i].col,
          sqlite3_errmsg(g_db));
    else
      changed = 1;
  }
  if (changed) {
    ...
    eh_store_set_cursor(0);
    eh_store_set_meta("schema_version", "2");
    eh_LOG("[bookshelf] store: schema migrated; sync cursor reset\n");
  }
}
...
static int store_import_legacy(const char *legacy_path) {
  char *txt = eh_read_text_file(legacy_path);
  if (txt == NULL)
    return 0;
  cJSON *root = cJSON_Parse(txt);
  free(txt);
  if (root == NULL) {
    eh_LOG("[bookshelf] store: legacy import: JSON parse failed\n");
    return -1;
  }
  int count = 0;
  int failed = 0;
  ...
  if (sqlite3_exec(g_db, "BEGIN", NULL, NULL, NULL) != SQLITE_OK) {
    eh_LOG("[bookshelf] store: legacy import BEGIN failed: %s\n",
        sqlite3_errmsg(g_db));
    cJSON_Delete(root);
    return -1;
  }
  if (cJSON_IsArray(root)) {
    ...
    cJSON_ArrayForEach(it, root) {
      ...
      if (eh_parse_book_obj(it, &tmp, 1) == 0 && eh_store_upsert_book(&tmp) == 0)
        count++;
      else
        failed = 1;
    }
...
static int store_apply_schema(void) {
  char ver[8] = "";
  if (eh_store_meta_value("schema_version", ver, sizeof ver) != 1 ||
      strcmp(ver, "2") != 0) {
  ...
  if (eh_store_meta_value("schema_version", ver, sizeof ver) != 1 ||
    ...
    eh_store_set_meta("schema_version", "3");
  }
  ...
  if (sqlite3_exec(g_db, SCHEMA_SQL, NULL, NULL, NULL) != SQLITE_OK) {
    ...
    store_migrate_columns();
    if (sqlite3_exec(g_db, SCHEMA_SQL, NULL, NULL, NULL) != SQLITE_OK) {
      eh_LOG("[bookshelf] store: schema failed: %s\n", sqlite3_errmsg(g_db));
      sqlite3_close(g_db);
      g_db = NULL;
      return 0;
    }
...
static void store_init_fts(void) {
  ...
  if (sqlite3_exec(g_db,
    ...
    g_no_fts = 1;
    eh_LOG("[bookshelf] store: FTS5 unavailable (%s); search falls back to LIKE\n",
        sqlite3_errmsg(g_db));
  }
}
...
static void store_import_legacy_once(void) {
  char legacy[EH_MAX_PATH_LEN * 2];
  char dir[EH_MAX_PATH_LEN];
  eh_dirname_of(eh_g_config_path, dir, sizeof dir);
  snprintf(legacy, sizeof legacy, "%s/%s", dir, EH_LIB_LEGACY_FILENAME);
  ...
  if (f != NULL) {
    ...
    if (n < 0) {
      ...
      eh_LOG("[bookshelf] store: legacy import incomplete, keeping %s\n",
          legacy);
    } else {
      char migrated[EH_MAX_PATH_LEN * 2 + 16];
      snprintf(migrated, sizeof migrated, "%s.migrated", legacy);
      rename(legacy, migrated);
      eh_LOG("[bookshelf] store: migrated legacy JSON (%d books)\n", n);
    }
  }
}

void eh_store_open(void) {
  char path[EH_MAX_PATH_LEN * 2];
  store_path(path, sizeof path);

  if (sqlite3_open(path, &g_db) != SQLITE_OK) {
    eh_LOG("[bookshelf] store: open failed %s: %s\n", path,
...
static void store_fts_backfill_if_empty(void) {
  ...
  if (sqlite3_exec(g_db,
                   ...
                   NULL, NULL, NULL) != SQLITE_OK)
    eh_LOG("[bookshelf] store: FTS backfill failed: %s\n", sqlite3_errmsg(g_db));
}
...
int eh_store_upsert_book(const BsBook *b) {
  ...
  if (rc != SQLITE_DONE)
    eh_LOG("[bookshelf] upsert FAILED id=%s rc=%d: %s\n", b->id, rc,
        sqlite3_errmsg(g_db));
  else
...
void eh_store_local_meta_put(const char *id, const char *title,
  ...
  int rc = sqlite3_step(st);
  if (rc != SQLITE_DONE)
    eh_LOG("[bookshelf] store_local_meta_put failed: %s\n", sqlite3_errmsg(g_db));
}
...
void eh_store_set_downloaded(const char *id, int downloaded,
  ...
  int rc = sqlite3_step(st);
  if (rc != SQLITE_DONE)
    eh_LOG("[bookshelf] store_set_downloaded failed: %s\n", sqlite3_errmsg(g_db));
}
...
int eh_store_get_book(const char *id, BsBook *out) {
  if (g_db == NULL)
    return 0;
  sqlite3_stmt *st = st_prep_once(
      &g_st_get_book, "SELECT " EH_BOOK_COLS " FROM books WHERE id=?1");
  if (st == NULL)
    return 0;
  sqlite3_reset(st);
  sqlite3_clear_bindings(st);
...
int eh_store_next_undownloaded(char ids[][EH_MAX_ID_LEN], int cap) {
  ...
  while (n < cap && sqlite3_step(st) == SQLITE_ROW) {
    ...
    snprintf(ids[n], EH_MAX_ID_LEN, "%s", id ? id : "");
    n++;
  }
  sqlite3_finalize(st);
  return n;
 ...
 * pattern as eh_store_next_dl_probes): the caller keeps *after_rowid
...
void eh_store_delete_book_file(const char *id) {
  ...
  if (unlink(path) == 0)
    eh_LOG("[bookshelf] delete_book_file removed %s\n", path);
  else
    eh_LOG("[bookshelf] delete_book_file unlink failed %s\n", path);
  eh_store_set_downloaded(id, 0, "");
  BsDownloadItem *d = eh_find_download(id);
  if (d != NULL)
    d->state = 3;
}
...
void eh_store_search_add(const char *term) {
  ...
  if (sqlite3_prepare_v2(g_db,
    ...
    sqlite3_bind_int(trim, 1, EH_SEARCH_HISTORY_MAX);
    sqlite3_step(trim);
...
int eh_store_search_list(char terms[][EH_MAX_QUERY_LEN], int cap, int offset) {
  ...
  while (n < cap && sqlite3_step(st) == SQLITE_ROW) {
    const char *t = (const char *)sqlite3_column_text(st, 0);
    snprintf(terms[n], EH_MAX_QUERY_LEN, "%s", t ? t : "");
    n++;
  }
  sqlite3_finalize(st);
  return n;
...
static void suggest_snapshot_old(const char *book_id,
                                 char old[][EH_SUGGEST_TERM_MAX], int max,
                                 int *n_old) {
  sqlite3_stmt *q = NULL;
  if (sqlite3_prepare_v2(g_db, "SELECT term FROM suggest WHERE book_id=?1",
    ...
    while (*n_old < max && sqlite3_step(q) == SQLITE_ROW) {
      ...
      snprintf(old[*n_old], EH_SUGGEST_TERM_MAX, "%s", t ? t : "");
      (*n_old)++;
    }
    sqlite3_finalize(q);
  }
...
static void suggest_insert_terms(sqlite3_stmt *del, const char *book_id,
                                 const char terms[][EH_SUGGEST_TERM_MAX],
                                 int n) {
  sqlite3_reset(del);
 ...
 * all. */
static void suggest_refresh_rank(const char old[][EH_SUGGEST_TERM_MAX],
                                 int n_old,
                                 const char terms[][EH_SUGGEST_TERM_MAX],
                                 int n) {
...
void eh_store_suggest_set(const char *book_id,
  ...
  if (g_db == NULL || book_id == NULL)
    return;
  char old[EH_SUGGEST_MAX_TERMS][EH_SUGGEST_TERM_MAX];
  int n_old = 0;
  suggest_snapshot_old(book_id, old, EH_SUGGEST_MAX_TERMS, &n_old);
...
int eh_store_suggest_list(const char *prefix, char out[][EH_SUGGEST_TERM_MAX],
  ...
  if (len < 2 || len >= EH_SUGGEST_TERM_MAX)
    return 0;
  char norm[EH_SUGGEST_TERM_MAX];
  suggest_lower(norm, prefix, len);
  char bound[EH_SUGGEST_TERM_MAX + 4];
  ...
  while (n < cap && sqlite3_step(st) == SQLITE_ROW) {
    const char *t = (const char *)sqlite3_column_text(st, 0);
    snprintf(out[n], EH_SUGGEST_TERM_MAX, "%s", t ? t : "");
    n++;
  }
  sqlite3_finalize(st);
  if (n == 0 && ranked) {
...
int eh_store_begin(void) {
  ...
  if (sqlite3_exec(g_db, "BEGIN", NULL, NULL, &msg) != SQLITE_OK) {
    eh_LOG("[bookshelf] store_begin FAILED: %s\n",
...
int eh_store_commit(void) {
  if (g_db != NULL) {
    ...
    if (sqlite3_exec(g_db, "COMMIT", NULL, NULL, &msg) != SQLITE_OK) {
      eh_LOG("[bookshelf] store_commit FAILED: %s\n",
          msg ? msg : sqlite3_errmsg(g_db));
...
void eh_store_rollback(void) {
  if (g_db != NULL) {
    ...
    if (sqlite3_exec(g_db, "ROLLBACK", NULL, NULL, &msg) != SQLITE_OK) {
      eh_LOG("[bookshelf] store_rollback FAILED: %s\n",
          msg ? msg : sqlite3_errmsg(g_db));
      if (msg)
        sqlite3_free(msg);
    }
...
static const char *view_source(void) {
  return eh_g_state.source == EH_SOURCE_LOCAL    ? "local"
...
static void search_fts_decide(void) {
  if (strcmp(g_search_q_cache, eh_g_state.query) == 0)
    return;
  snprintf(g_search_q_cache, sizeof g_search_q_cache, "%s", eh_g_state.query);
  g_search_use_fts = 0;
  g_fts_query[0] = '\0';
  if (g_no_fts || eh_g_state.query[0] == '\0' || g_db == NULL)
    return;
  if (fts_query_from(eh_g_state.query, g_fts_query, sizeof g_fts_query) <= 0)
    return;
...
static void view_where(char *sql, size_t cap, int qbind) {
  ...
  if (qbind > 0 && eh_g_state.query[0] != '\0') {
    search_fts_decide();
    if (g_search_use_fts) {
...
static void view_bind_query(sqlite3_stmt *st, int qbind) {
  ...
  char pat[EH_MAX_QUERY_LEN * 2 + 4];
  char esc[EH_MAX_QUERY_LEN * 2];
  like_escape(eh_g_state.query, esc, sizeof esc);
  snprintf(pat, sizeof pat, "%%%s%%", esc);
  bind_text_trunc(st, qbind, pat);
}

/* ── dimension grouping support ──────────────────────────────────────── *
 * A chosen grouping dimension (eh_g_group_dim) collapses the view into
...
static int group_levels(BsGroupPreset g) {
  switch (g) {
  case EH_GROUP_AUTHOR_SERIES: return 2;
  case EH_GROUP_NONE:          return 0;
  default:                     return 1;
  }
}
...
static BsGroupDim dim_at(BsGroupPreset g, int lvl) {
  switch (g) {
  case EH_GROUP_SERIES:        return EH_GROUP_BY_SERIES;
  case EH_GROUP_AUTHOR:        return EH_GROUP_BY_AUTHOR;
  case EH_GROUP_YEAR:          return EH_GROUP_BY_YEAR;
  case EH_GROUP_GENRE:         return EH_GROUP_BY_GENRE;
  case EH_GROUP_AUTHOR_SERIES: return lvl == 0 ? EH_GROUP_BY_AUTHOR
                                               : EH_GROUP_BY_SERIES;
  default:                     return EH_GROUP_ALL;
  }
}

/* 1 = a dimension grouping is active (drilling beneath the top). */
static int grouped_active(void) {
  return eh_g_group != EH_GROUP_NONE &&
         eh_g_drill_level < group_levels(eh_g_group);
}

/* SQL for a grouping dimension; q selects the view-JOIN alias (`b.`). */
static const char *dim_sql(BsGroupDim dim, int q) {
  switch (dim) {
  case EH_GROUP_BY_SERIES:
...
static void group_label_for(BsGroupDim dim, const char *value, char *out,
  ...
  if (value[0] == '\0') {
    const char *kl = NULL;
    switch (dim) {
    case EH_GROUP_BY_SERIES: kl = "group.none.series"; break;
    case EH_GROUP_BY_AUTHOR: kl = "group.none.author"; break;
    case EH_GROUP_BY_YEAR:   kl = "group.none.year";   break;
    case EH_GROUP_BY_GENRE:  kl = "group.none.genre";  break;
    default: break;
    }
    snprintf(out, cap, "%s", kl ? eh_i18n(kl) : "...");
  } else {
...
int eh_view_dim_available(BsGroupDim dim) {
  ...
  if (dim == EH_GROUP_BY_SERIES && eh_g_state.source != EH_SOURCE_KAVITA)
    return 0;
...
static void view_append_drill_conds(char *sql, size_t cap) {
  for (int L = 0; L < eh_g_drill_level && L < EH_GROUP_MAX_LEVELS; L++) {
    const char *e = dim_sql(dim_at(eh_g_group, L), 0);
    if (eh_g_drill_values[L][0])
...
static int view_run_scoped_select(const char *sql, int *inserted) {
  ...
  if (rc == SQLITE_OK) {
    for (int L = 0; L < eh_g_drill_level; L++)
      if (eh_g_drill_values[L][0])
        bind_text_trunc(st, L + 1, eh_g_drill_values[L]);
    view_bind_query(st, eh_g_drill_level + 1);
    rc = sqlite3_step(st);
    if (inserted != NULL)
...
static int view_rebuild_group(int *inserted) {
  const char *dim = dim_sql(dim_at(eh_g_group, eh_g_drill_level), 0);
  /* The card label is the display value (series name for series,
   * which groups by the API series_id). */
  const char *lbl = (dim_at(eh_g_group, eh_g_drill_level) == EH_GROUP_BY_SERIES)
                        ? "series"
                        : dim;
  view_drop_temps();
  int rc = sqlite3_exec(g_db,
                        ...
                        NULL, NULL, NULL);
  if (rc != SQLITE_OK)
    eh_LOG("[bookshelf] view_rebuild: t_sorted create rc=%d: %s\n", rc,
        sqlite3_errmsg(g_db));
  if (rc == SQLITE_OK) {
    char sql[2048];
    snprintf(sql, sizeof sql,
             ...
             lbl);
    view_where(sql, sizeof sql, eh_g_drill_level + 1);
...
void eh_view_rebuild(void) {
  ...
  int prev_total = eh_g_view_total;

  if (sqlite3_exec(g_db, "BEGIN", NULL, NULL, NULL) != SQLITE_OK) {
    eh_LOG("[bookshelf] view_rebuild: BEGIN failed: %s\n",
        sqlite3_errmsg(g_db));
    return; /* previous view and g_view_total stay intact */
  }
  if (sqlite3_exec(g_db, "DELETE FROM view", NULL, NULL, NULL) != SQLITE_OK) {
    eh_LOG("[bookshelf] view_rebuild: DELETE failed: %s\n",
        sqlite3_errmsg(g_db));
    sqlite3_exec(g_db, "ROLLBACK", NULL, NULL, NULL);
    return; /* previous view and g_view_total stay intact */
  }
  ...
  eh_g_view_source = eh_g_state.source;
  eh_g_view_total = inserted;
  eh_LOG("[bookshelf] view_rebuild: view=%d sort=%d group=%d drill=%d\n",
      eh_g_view_total, (int)eh_g_state.sort, (int)eh_g_group,
      eh_g_drill_level);
}
...
int eh_view_fetch_page(int page, BsTileRow *rows, int cap) {
  ...
  if (sqlite3_prepare_v2(g_db, sql, -1, &st, NULL) != SQLITE_OK) {
    eh_LOG("[bookshelf] view_fetch_page PREPARE FAIL: %s\n", sqlite3_errmsg(g_db));
    return 0;
  }
  ...
  if (grouped_active() && n > 0) {
    BsGroupDim d = dim_at(eh_g_group, eh_g_drill_level);
    for (int i = 0; i < n; i++)

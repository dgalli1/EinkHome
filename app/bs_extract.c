...
xml_unescape(char *s)
{
    ...
    for (char *r = s; *r;) {
        ...
        if (!done && r[0] == '&' && r[1] == '#') {
            /* &#NN; — decode any codepoint, UTF-8-encoded on output. */
            char *end = NULL;
...
zip_entry_read(const char *zip_path, const char *want, unsigned char **out, size_t *out_len)
{
    ...
    /* End-of-central-directory: scan the last 64 KiB back for PK\x05\x06. */
    ...
    /* Walk the central directory for the wanted name. */
    size_t        pos = 0;
          ...
          size_t      cover_cap)
{
    ...
    if (zip_entry_read(path, "META-INF/container.xml", &xml, &xlen) != 0)
        return -1;
    char opf[512] = "";
    xml_attr((const char *)xml, "full-path", opf, sizeof opf);
    free(xml);
    if (opf[0] == '\0')
        return -1;
    if (opf[0] == '/')
    ...
    if (cover_out != NULL) {
        ...
        while ((p = strstr(p, "<meta")) != NULL) {
            ...
            if (xml_attr(tag, "name", nv, sizeof nv) == 0 && strcmp(nv, "cover") == 0) {
                if (xml_attr(tag, "content", cid, sizeof cid) != 0)
                    xml_attr(tag, "content", cid, sizeof cid);
                break;
            }
            p = gt + 1;
        }
        if (cid[0] != '\0') {
            ...
            const char *it = strstr((const char *)xml, "<item");
            while (it != NULL) {
                ...
                if (strstr(tag, want) != NULL) {
                    ...
                    if (xml_attr(tag, "href", href, sizeof href) == 0) {
                        ...
                        size_t         ilen = 0;
                        if (zip_entry_read(path, entry, &img, &ilen) == 0) {
...
int
extract_book_meta(const char *path,
                  const char *ext,
                  char       *title,
                  size_t      title_cap,
                  char       *author,
...
int
extract_book_cover(const char *path, const char *ext, char *out_path, size_t out_cap)
{
    if (ext == NULL)
        return -1;
    if (strcmp(ext, "epub") == 0)

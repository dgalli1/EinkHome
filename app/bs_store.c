...
/* The stock bookshelf creates the file explorer's database on first
 * boot (pocketbook::DbCreator::GetExplorerDb in libframework2); the
 * built-in reader opens it in EinkDbInterfaceImpl and aborts with
 * "unable to open database file" when it is missing.  EinkHome
 * replaces the stock bookshelf as the home task, so it provides the
 * database itself.  Only the core tables are created — the reader's
 * book-open path works on an empty database, and the schema is what
 * the firmware's own creator would write. */
void ensure_explorer_db(void) {
  static const char *const path = "/mnt/ext1/system/explorer-3/explorer-3.db";
  if (access(path, R_OK | W_OK) == 0)
    return;
  mkdir("/mnt/ext1/system/explorer-3", 0777); /* best-effort */

  sqlite3 *db = NULL;
  if (sqlite3_open(path, &db) != SQLITE_OK) {
    LOG("[bookshelf] explorer db: cannot create %s: %s\n", path,
  ...
  static const char *const schema[] = {
      ...
      " id INTEGER PRIMARY KEY, type INTEGER, name TEXT, sync_account_id "
      "INTEGER)",
      "CREATE TABLE IF NOT EXISTS folders("
      " id INTEGER PRIMARY KEY AUTOINCREMENT, storageid INTEGER, name TEXT,"
      " FOREIGN KEY(storageid) REFERENCES storages(id),"
      " UNIQUE(storageid, name) ON CONFLICT FAIL)",
      "CREATE TABLE IF NOT EXISTS books_impl("
      " id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, first_title_letter "
      "TEXT,"
      " author TEXT, firstauthor TEXT, first_author_letter TEXT, series TEXT,"
      " numinseries INTEGER, size INTEGER, creationtime INTEGER NOT NULL "
      "DEFAULT 0,"
      " updated INTEGER NOT NULL DEFAULT 0, date_added TEXT, ts_added INTEGER,"
      " isbn TEXT, hidden INTEGER NOT NULL DEFAULT 0, sort_title TEXT, "
      "resource_id TEXT)",
      "CREATE TABLE IF NOT EXISTS files("
      " id INTEGER PRIMARY KEY AUTOINCREMENT, storageid INTEGER, folder_id "
      "INTEGER,"
      " book_id INTEGER, filename TEXT, size INTEGER, modification_time "
      "INTEGER,"
      " fast_hash TEXT, ext TEXT,"
      " FOREIGN KEY(storageid) REFERENCES storages(id),"
      " FOREIGN KEY(folder_id) REFERENCES folders(id),"
      " FOREIGN KEY(book_id) REFERENCES books_impl(id))",
      "CREATE TABLE IF NOT EXISTS books_settings("
      " bookid INTEGER, profileid INTEGER, position TEXT, position_ts INTEGER,"
      " cpage INTEGER, npage INTEGER, opentime INTEGER, completed INTEGER,"
      " completed_ts INTEGER, favorite INTEGER, favorite_ts INTEGER, custom "
      "TEXT,"
      " PRIMARY KEY(bookid, profileid),"
      " FOREIGN KEY(bookid) REFERENCES books_impl(id),"
      " FOREIGN KEY(profileid) REFERENCES profiles(id))",
      "CREATE TABLE IF NOT EXISTS file_props("
      " fast_hash TEXT, prop_type INTEGER, value TEXT,"
      " PRIMARY KEY(fast_hash, prop_type))",
      "CREATE TABLE IF NOT EXISTS books_fast_hashes(fast_hash TEXT PRIMARY "
      "KEY, book_id INTEGER)",
      "CREATE TABLE IF NOT EXISTS genres(id INTEGER PRIMARY KEY AUTOINCREMENT,"
      " name TEXT, UNIQUE(name))",
      "CREATE TABLE IF NOT EXISTS booktogenre("
      " bookid INTEGER, genreid INTEGER, PRIMARY KEY(bookid, genreid),"
      " FOREIGN KEY(bookid) REFERENCES books_impl(id),"
      " FOREIGN KEY(genreid) REFERENCES genres(id))",
      "CREATE TABLE IF NOT EXISTS social("
      " bookid INTEGER, profileid INTEGER, completed INTEGER, reading INTEGER,"
      " liked INTEGER, status INTEGER, stars INTEGER,"
      " PRIMARY KEY(bookid, profileid),"
      " FOREIGN KEY(bookid) REFERENCES books_impl(id),"
      " FOREIGN KEY(profileid) REFERENCES profiles(id))",
      "CREATE TABLE IF NOT EXISTS bookshelfs("
      " id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, is_deleted INTEGER,"
      " ts INTEGER, uuid TEXT, UNIQUE(name) ON CONFLICT FAIL)",
      "CREATE TABLE IF NOT EXISTS bookshelfs_books("
      " bookshelfid INTEGER, bookid INTEGER, ts INTEGER, is_deleted INTEGER,"
      ...
      " timestampid INTEGER NOT NULL REFERENCES timestamps(id),"
      " type INTEGER NOT NULL,"
      " profileid INTEGER NOT NULL REFERENCES profiles(id),"
      " title TEXT, path TEXT, author TEXT, recommendator TEXT, is_viewed "
      "INTEGER,"
      " params TEXT, notification_app_id TEXT,"
      " PRIMARY KEY(timestampid, type, profileid))",
      "CREATE TABLE IF NOT EXISTS sync_accounts("
      " id INTEGER PRIMARY KEY, name TEXT, sync_cursor TEXT, sync_ts INTEGER,"
      " is_active INTEGER)",
      "CREATE TABLE IF NOT EXISTS sync_data("
      " b_id INTEGER, profile_id INTEGER, data_type INTEGER, ts INTEGER,"
      " PRIMARY KEY(b_id, profile_id, data_type))",
      "CREATE TABLE IF NOT EXISTS migrated(id INTEGER PRIMARY KEY)",
  };
  /* One transaction: the file explorer's boot task vacuums/checks
   * this db concurrently, and per-statement commits would let it read
   * a half-written file (its corruption handler then requests a guest
   * reboot).  A single transaction holds the write lock for the whole
   * creation, so a concurrent reader sees "busy", not "malformed". */
  int failed = 0;
  if (sqlite3_exec(db, "BEGIN", NULL, NULL, NULL) != SQLITE_OK)
    failed = 1;
  for (size_t i = 0; i < sizeof schema / sizeof schema[0] && !failed; i++) {
    if (sqlite3_exec(db, schema[i], NULL, NULL, NULL) != SQLITE_OK) {
      LOG("[bookshelf] explorer db: schema step %zu failed: %s\n", i,
          sqlite3_errmsg(db));
      failed = 1;
    }
  }
  ...
  sqlite3_close(db);
  /* The reader may run under a different uid than this app. */
  chmod(path, 0666);
  LOG("[bookshelf] explorer db: %s %s\n", path,
      failed ? "incomplete" : "ready");
}

/* eh_worker.c — shared one-shot background jobs (see eh_worker.h).
 *
...
eh_worker_ensure(void)
{
    ...
    if (!atomic_load_explicit(&g_worker_started, memory_order_acquire)) {
        pthread_t t;
        if (pthread_create(&t, NULL, eh_worker_main, NULL) == 0) {
            pthread_detach(t);
...
BsJob *
eh_worker_submit(eh_job_fn fn, eh_job_done done_cb, void *arg)
{
    ...
    eh_worker_ensure();
    SetWeakTimerEx("wkr", eh_worker_timer_cb, NULL, 30);
    return job;
}

void
eh_worker_cancel(BsJob *job)
{
    if (job != NULL)

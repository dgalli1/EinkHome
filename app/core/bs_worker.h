/* bs_worker.h — one-shot background jobs with main-thread completion.
 *
...
typedef void (*bs_job_fn)(BsJob *job);    /* worker thread */
typedef void (*bs_job_done)(BsJob *job);  /* main thread */
...
BsJob *bs_worker_submit(bs_job_fn fn, bs_job_done done_cb, void *arg);

/* bs_config.c — part of the bookshelf app (see bs_core.h) */
...
int
bs_read_kv_file(const char *path, bs_cfg_kv_cb cb, void *user)
{
    FILE *f = fopen(path, "r");
    if (f == NULL)
        return -1;
    ...
    while (fgets(line, sizeof line, f) != NULL) {
        lineno++;
        char *p = bs_trim_ws(line);
        if (*p == '\0' || *p == '#' || *p == ';')
            continue;
        char *eq = strchr(p, '=');
        if (eq == NULL) {
            bs_LOG("[bookshelf] %s:%d: ignoring `%s`\n", path, lineno, p);
            continue;
        }
        *eq = '\0';
        char *k = bs_trim_ws(p);
        char *v = bs_trim_ws(eq + 1);
        cb(k, v, user);
    }
    fclose(f);
    return 0;
...
void
bs_dirname_of(const char *path, char *out, size_t out_cap)
{
...
void
bs_load_config_file(const char *argv0, struct bs_cfg_out *out)
{
    snprintf(out->api_token, out->token_cap, "%s", BS_TOKEN_DEFAULT);
    char path[512];

    if (argv0 != NULL && strchr(argv0, '/') != NULL) {
        bs_dirname_of(argv0, path, sizeof path);
        if (path[0] != '\0') {
            char candidate[512];
            snprintf(candidate, sizeof candidate, "%s/bookshelf.cfg", path);
            if (bs_read_kv_file(candidate, bs_cfg_set_kv, out) == 0)
                bs_LOG("[bookshelf] config: %s\n", candidate);
        }
    }
    if (bs_read_kv_file("/etc/pbemu/bookshelf.cfg", bs_cfg_set_kv, out) == 0)
        bs_LOG("[bookshelf] config: /etc/pbemu/bookshelf.cfg\n");
    ...
    if (bs_read_kv_file(BS_CONFIG_TMP_PATH, bs_cfg_set_kv, out) == 0)
        bs_LOG("[bookshelf] config: %s (override)\n", BS_CONFIG_TMP_PATH);
}
...
char bs_g_config_path[600];

void
bs_resolve_config_path(const char *argv0)
{
    char primary[600];
    primary[0] = '\0';
    if (argv0 != NULL && strchr(argv0, '/') != NULL) {
        char dir[512];
        bs_dirname_of(argv0, dir, sizeof dir);
        if (dir[0] != '\0')
            snprintf(primary, sizeof primary, "%s/%s", dir, BS_CONFIG_FILENAME);
    }
    if (primary[0] == '\0')
        snprintf(primary, sizeof primary, "/etc/pbemu/%s", BS_CONFIG_FILENAME);
    ...
    if (dir_copy[0] != '\0' && access(dir_copy, W_OK) == 0) {
        snprintf(bs_g_config_path, sizeof bs_g_config_path, "%s", primary);
        return;
    }
    snprintf(bs_g_config_path, sizeof bs_g_config_path, "%s", BS_CONFIG_TMP_PATH);
}

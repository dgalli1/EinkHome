/* eh_logview.c — part of the bookshelf app (see eh_core.h) */
              ...
              int *count, const char **ws)
{
    ...
    int curw = dst[*count].len > 0 ? eh_span_width(dst[*count].p, dst[*count].len) : 0;
...
eh_draw_log_view(void)
{
     ...
     * the centred title). */
    eh_draw_overlay_header(eh_i18n("log.title"));
    ifont *pf = OpenFont(DEFAULTFONT, 20, 0);
    if (pf != NULL) {
        SetFont(pf, DGRAY);
        char shown[200];
        snprintf(shown, sizeof shown, "%s", eh_log_path());
     ...
     * app; observed only on the 1072x1448 U634k3, where body_h/EH_LOG_ROW_H
    ...
    if (wc == NULL) {
        ...
        if (ef != NULL) {
            SetFont(ef, DGRAY);
            DrawString(32, body_top + 40, eh_i18n("log.empty"));
            CloseFont(ef);
        }
    } else {
         ...
         * open.  Only an explicit scroll (see eh_on_tap_log_view)
         * materialises a concrete first-line index. */
        if (eh_g_state.log_scroll < 0) {
            first = max_first;
        } else {
            first = eh_g_state.log_scroll;
            if (first > max_first)
                first = max_first;
            if (first < 0)
                first = 0;
            eh_g_state.log_scroll = first;
        }

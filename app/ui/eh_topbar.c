/* eh_topbar.c — part of the bookshelf app (see eh_core.h) */
...
void
eh_draw_back_icon(int cx, int cy, int col)
{
    int ax = cx - 8;
    int ay = cy;
...
{
    int cx = x + EH_TOP_ICON_HALF, cy = y + EH_TOP_ICON_HALF, r = 24;
    eh_draw_circle_outline(cx, cy, r, col);
    /* equator */
    int px = 0, py = 0;
    for (int s = 0; s <= 16; s++) {
...
{
    int cx = x + EH_TOP_ICON_HALF, cy = y + EH_TOP_ICON_HALF;
...
{
    switch (eh_g_state.source) {
    case EH_SOURCE_LOCAL:
        return eh_i18n("source.local");
    case EH_SOURCE_FOLDER:
        return eh_i18n("source.folder");
    default:
        return eh_i18n("source.kavita");
    }
}
...
{
    int col = BLACK;
    int x0 = EH_SOURCE_BTN_X;
    int w_btn = eh_source_btn_w();
    FillArea(x0, 0, w_btn, EH_TOP_BAR_H, WHITE);
    int cy = EH_TOP_BAR_H / 2;
...
top_bar_title(char *title, size_t cap)
{
    if (eh_g_state.tab == EH_TAB_SEARCH) {
        snprintf(title, cap, "%s", eh_i18n("tab.search"));
         ...
         * user). */
        char shown[96];
        eh_user_path_display(eh_g_browse_path, shown, sizeof shown);
        size_t plen = strlen(shown);
        if (plen > cap - 1)
            plen = cap - 1;
        memcpy(title, shown, plen);
        title[plen] = '\0';
    } else if (eh_g_state.query[0] != '\0') {
        snprintf(title, cap, "%s", eh_g_state.query);
    } else {
        title[0] = '\0';
    }
}
                   ...
                   char *title, size_t tsiz)
{
    if (eh_g_state.tab == EH_TAB_SEARCH) {
         ...
         * centred. */
        int guard = home_x + home_w + EH_TOP_BTN_PAD;
        int budget = w - 2 * guard;
        eh_utf8_fit_width(title, tsiz, budget);
        SetFont(tf, col);
        DrawString((w - StringWidth(title)) / 2, y0 + (EH_TOP_BAR_H - 40) / 2, title);
    } else {
        ...
        int left_w = EH_TOP_BTN_PAD + EH_TOP_BTN_SIZE + EH_TOP_BTN_PAD + eh_source_btn_w();
        int right_w = EH_TOP_BTN_PAD + 4 * EH_TOP_BTN_SIZE;
        int band_w = w - left_w - right_w;
        if (band_w < 64)
            band_w = 64;
        eh_utf8_fit_width(title, tsiz, band_w);
        SetFont(tf, col);
        DrawString(left_w + (band_w - StringWidth(title)) / 2,
                   y0 + (EH_TOP_BAR_H - 40) / 2,
                   title);
    }
}

void
eh_draw_top_bar(void)
{
    int w = ScreenWidth();
    int y0 = 0; /* top bar sits at the very top; the system panel is at the bottom */
    int col = BLACK;

    FillArea(0, y0, w, EH_TOP_BAR_H, WHITE);
    DrawLine(0, y0 + EH_TOP_BAR_H, w, y0 + EH_TOP_BAR_H, col);
    ...
    int home_w = EH_TOP_BTN_SIZE;
    int home_x = EH_TOP_BTN_PAD;
    int home_y = y0 + (EH_TOP_BAR_H - home_w) / 2;
    int hcx = home_x + home_w / 2;
    int hcy = home_y + home_w / 2;
    if (eh_g_drill_level > 0 || eh_g_state.tab == EH_TAB_SEARCH) {
        eh_draw_back_icon(hcx, hcy, col);
    } else {
    ...
    if (tf != NULL) {
        char title[EH_MAX_QUERY_LEN + 16];
    ...
    int menu_w = EH_TOP_BTN_SIZE;
    int menu_x = w - menu_w - EH_TOP_BTN_PAD;
    int menu_y = y0 + (EH_TOP_BAR_H - menu_w) / 2;
    ...
    DrawLine(EH_TOP_BTN_PAD + EH_TOP_BTN_SIZE + 4, y0, EH_TOP_BTN_PAD + EH_TOP_BTN_SIZE + 4, y0 + EH_TOP_BAR_H, col);
    DrawLine(EH_SOURCE_BTN_X + eh_source_btn_w(), y0, EH_SOURCE_BTN_X + eh_source_btn_w(), y0 + EH_TOP_BAR_H, col);
    DrawLine(w - (EH_TOP_BTN_PAD + 4 * EH_TOP_BTN_SIZE), y0, w - (EH_TOP_BTN_PAD + 4 * EH_TOP_BTN_SIZE), y0 + EH_TOP_BAR_H, col);
    DrawLine(w - (EH_TOP_BTN_PAD + 3 * EH_TOP_BTN_SIZE), y0, w - (EH_TOP_BTN_PAD + 3 * EH_TOP_BTN_SIZE), y0 + EH_TOP_BAR_H, col);
    DrawLine(w - (EH_TOP_BTN_PAD + 2 * EH_TOP_BTN_SIZE), y0, w - (EH_TOP_BTN_PAD + 2 * EH_TOP_BTN_SIZE), y0 + EH_TOP_BAR_H, col);
    DrawLine(w - (EH_TOP_BTN_PAD + EH_TOP_BTN_SIZE), y0, w - (EH_TOP_BTN_PAD + EH_TOP_BTN_SIZE), y0 + EH_TOP_BAR_H, col);
}
...
sync_active(void)
{
    return eh_g_state.sync_state == 1 || eh_downloads_pending() > 0 || eh_g_dl_batch_active;
}
...
eh_modal_open(void)
{
    return eh_g_state.overlay != EH_OV_NONE || eh_g_state.dl_popup || eh_g_state.sync_popup;
}
...
sync_spin_tick(void *ctx)
{
    ...
    eh_g_state.sync_angle = (eh_g_state.sync_angle + 15) % 360;
    /* The glyph only exists on the Library tab; elsewhere the top bar
     * is redrawn whole when the state that feeds it changes. */
    if (!eh_modal_open() && eh_g_state.tab != EH_TAB_SEARCH) {
        eh_draw_sync_icon();
        PartialUpdate(ScreenWidth() - EH_TOP_BTN_PAD - 2 * EH_TOP_BTN_SIZE, 0, EH_TOP_BTN_SIZE, EH_TOP_BAR_H);
    }
    SetWeakTimerEx("bspin", sync_spin_tick, NULL, 1000);
}

void
eh_sync_set_active(int on)
{
...
void
eh_draw_sync_icon(void)
{
    int w = ScreenWidth();
    int y0 = 0;
    int ic_w = EH_TOP_BTN_SIZE;
    int ic_x = w - ic_w - EH_TOP_BTN_PAD - ic_w; /* left of the menu button */
    int ic_y = y0 + (EH_TOP_BAR_H - ic_w) / 2;
    ...
    for (int half = 0; half < 2; half++) {
        int a0 = eh_g_state.sync_angle + half * 180;
        int px = 0, py = 0;
        int ex = 0, ey = 0;
        for (int s = 0; s <= 8; s++) {
...
void
eh_draw_layout_icon(void)
{
    int w = ScreenWidth();
    int y0 = 0;
    int col = BLACK;
    int ic_w = EH_TOP_BTN_SIZE;
    int ic_x = w - ic_w - EH_TOP_BTN_PAD - 2 * ic_w; /* left of the sync button */
    int ic_y = y0 + (EH_TOP_BAR_H - ic_w) / 2;
    FillArea(ic_x, ic_y, ic_w, ic_w, WHITE);
    int cx = ic_x + ic_w / 2;
    int cy = ic_y + ic_w / 2;
 ...
 * button. */
void
eh_draw_search_icon(void)
{
    int w = ScreenWidth();
    int y0 = 0;
    int col = BLACK;
    int ic_w = EH_TOP_BTN_SIZE;
    int menu_x = w - EH_TOP_BTN_SIZE - EH_TOP_BTN_PAD;
...
int
eh_downloads_pending(void)
{
    int n = 0;
    for (int i = 0; i < eh_g_download_count; i++) {
        if (eh_g_downloads[i].state == 0)
            n++;
        else if (eh_g_downloads[i].state == 1 && !eh_dl_fetch_idle())

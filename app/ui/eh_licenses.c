/* eh_licenses.c — third-party licenses viewer (Settings → Licenses).
 *
 * A full-screen overlay, shaped like the log viewer (eh_logview.c):
 * the shared overlay header plus a scrollable body and the stock corner
 * scroll buttons.  Two views share the overlay:
 *
 *   - LIST   (eh_g_state.lic_sel < 0): one row per bundled license —
 ...
 *
 * The license texts ship as C strings (eh_licenses.c), so this viewer
              ...
              int *count, const char **ws)
{
    ...
    int wordw = eh_span_width(*ws, (int)(we - *ws));
    int curw = rows[*count].len > 0 ? eh_span_width(rows[*count].p, rows[*count].len) : 0;
...
{
    eh_draw_overlay_header(eh_i18n("licenses.title"));
    int body_top = EH_LIC_LIST_TOP;
    ...
    if (max_first < 0)
        max_first = 0;
    int first = eh_g_state.lic_scroll;
    if (first > max_first)
        first = max_first;
    if (first < 0)
        first = 0;
    eh_g_state.lic_scroll = first;

    ifont *nf = OpenFont(DEFAULTFONTB, 30, 0);
    ifont *lf = OpenFont(DEFAULTFONT, 24, 0);
    for (int i = 0; i < rows_vis && first + i < n; i++) {
        const BsLicense *lic = eh_license(first + i);
        int             y = body_top + i * EH_LIC_LIST_H;
        FillArea(16, y, w - 32, EH_LIC_LIST_H - 12, WHITE);
        DrawRect(16, y, w - 32, EH_LIC_LIST_H - 12, BLACK);
        if (nf != NULL) {
            SetFont(nf, BLACK);
    ...
    if (nf != NULL)
        CloseFont(nf);
    eh_draw_scroll_buttons(first > 0, first < max_first);
}
...
{
    if (eh_g_state.lic_sel >= n)
        eh_g_state.lic_sel = n - 1;
    if (eh_g_state.lic_sel < 0)
        eh_g_state.lic_sel = 0;
    const BsLicense *lic = eh_license(eh_g_state.lic_sel);

    eh_draw_overlay_header(lic->name);
    ...
    if (pf != NULL) {
        ...
        eh_utf8_fit_width(shown, sizeof shown, w - 2 * 32);
        DrawString((w - StringWidth(shown)) / 2, EH_OVERLAY_HEADER_H + 10, shown);
        CloseFont(pf);
    }
    ...
    int btn_y = h - 8 - EH_SCROLL_BTN_H;
    int body_h = btn_y - body_top - 8;
    if (body_h < EH_LOG_ROW_H)
        body_h = EH_LOG_ROW_H;
    int rows_vis = body_h / EH_LOG_ROW_H;

    BsLicRow rows[EH_LIC_MAX_ROWS];
    memset(rows, 0, sizeof rows); /* blank/len sentinel needs zeroed rows */
    int nrows = lic_wrap_rows(lic->text, w - 48, rows, EH_LIC_MAX_ROWS);
    int maxf = nrows - rows_vis;
    if (maxf < 0)
        maxf = 0;
    int first = eh_g_state.lic_scroll > maxf ? maxf : eh_g_state.lic_scroll;
    if (first < 0)
        first = 0;
    eh_g_state.lic_scroll = first;

    ifont *lf = OpenFont(DEFAULTFONT, EH_LOG_FONT_PX, 0);
    if (lf != NULL) {
        SetFont(lf, BLACK);
        for (int i = 0; i < rows_vis && first + i < nrows; i++) {
            ...
            tmp[len] = '\0';
            eh_utf8_fit_width(tmp, sizeof tmp, w - 48);
            DrawString(24, body_top + i * EH_LOG_ROW_H, tmp);
        }
        CloseFont(lf);
    }
    eh_draw_scroll_buttons(first > 0, first < maxf);
}

void
eh_draw_licenses_view(void)
{
    int w = ScreenWidth();

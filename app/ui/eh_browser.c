/* eh_browser.c — part of the bookshelf app (see eh_core.h) */
...
#define EH_BROWSE_MAX_ENTRIES 512

int  eh_g_browse_open = 0;
char eh_g_browse_path[256];
int  eh_g_browse_scroll = 0;
int  eh_g_browser_drag = 0;
int  eh_g_browser_drag_y = 0;
int  eh_g_browser_moved = 0;
...
static char g_browse_names[EH_BROWSE_MAX_ENTRIES][EH_MAX_PATH_LEN];
static char g_browse_is_dir[EH_BROWSE_MAX_ENTRIES];
static int  g_browse_count = 0;

/* Extensions the shelf treats as book files.  Shared with the Local
 * source import (eh_local.c) — keep both callers in lockstep. */
static const char *const BOOK_EXTS[] = {
    "epub", "pdf", "mobi", "azw", "azw3", "fb2", "djvu", "txt", "cbr", "cbz"};

int
eh_is_book_ext(const char *ext)
{
    if (ext == NULL)
        return 0;
 ...
 * Single home of the algorithm; eh_local.c calls this. */
void
eh_hash_hex(const char *s, char out[9])
{
...
browser_rows_visible(void)
{
    ...
    if (s_bmode == BR_MODE_PICKER)
        avail = eh_content_bottom() - EH_FOLDER_LIST_TOP - EH_FOLDER_BTN_H - EH_FOLDER_BTN_PAD;
    else
        avail = eh_content_bottom() - (EH_TOP_BAR_H + EH_TOP_BAR_PAD) - 8;
    int rows = avail / EH_FOLDER_ROW_H;
...
browser_clamp_scroll(void)
{
    ...
    if (max_scroll < 0)
        max_scroll = 0;
    if (eh_g_browse_scroll < 0)
        eh_g_browse_scroll = 0;
    if (eh_g_browse_scroll > max_scroll)
        eh_g_browse_scroll = max_scroll;
    return max_scroll;
}
...
browser_entry_is_dir(const struct dirent *e, int *is_reg)
{
    ...
    if (e->d_type == DT_UNKNOWN || e->d_type == DT_LNK) {
        ...
        size_t plen = strlen(eh_g_browse_path);
        size_t nlen = strlen(e->d_name);
        if (plen + 1 + nlen < sizeof path) {
            memcpy(path, eh_g_browse_path, plen);
            path[plen] = '/';
...
browser_apply_order(int *order)
{
    for (int i = 0; i < g_browse_count; i++) {
        if (order[i] == i)
            continue;
        char name[EH_MAX_PATH_LEN];
        memcpy(name, g_browse_names[i], EH_MAX_PATH_LEN);
        int is_dir = g_browse_is_dir[i];
        int src = i;
        for (;;) {
            int dst = order[src];
            if (dst == i) {
                memcpy(g_browse_names[src], name, EH_MAX_PATH_LEN);
                g_browse_is_dir[src] = (char)is_dir;
            } else {
                memcpy(g_browse_names[src], g_browse_names[dst], EH_MAX_PATH_LEN);
...
browser_load(void)
{
    ...
    if (s_bmode == BR_MODE_BROWSER && browser_can_go_up()) {
        snprintf(g_browse_names[0], EH_MAX_PATH_LEN, "..");
        g_browse_is_dir[0] = 1;
        g_browse_count = 1;
    }

    int max_entries =
        s_bmode == BR_MODE_PICKER ? EH_FOLDER_MAX_DIRS : EH_BROWSE_MAX_ENTRIES;
    DIR *d = opendir(eh_g_browse_path);
    if (d == NULL) {
        eh_LOG("[bookshelf] browser: opendir %s failed errno=%d\n", eh_g_browse_path, errno);
        return;
    }
    struct dirent *e;
    while ((e = readdir(d)) != NULL && g_browse_count < max_entries) {
        ...
        size_t nlen = strlen(e->d_name);
        if (nlen >= EH_MAX_PATH_LEN)
            nlen = EH_MAX_PATH_LEN - 1;
    ...
    int order[EH_BROWSE_MAX_ENTRIES];
    for (int i = 0; i < g_browse_count; i++)
        order[i] = i;
    ...
    eh_LOG("[bookshelf] browser: %s -> %d entries\n", eh_g_browse_path, g_browse_count);
}
...
{
    int w = ScreenWidth();
    FillArea(0, row_y, w, EH_FOLDER_ROW_H, WHITE);
    DrawLine(0, row_y + EH_FOLDER_ROW_H, w, row_y + EH_FOLDER_ROW_H, LGRAY);
    if (name == NULL)
        return;
    if (f != NULL) {
        SetFont(f, BLACK);
        char trunc[EH_MAX_PATH_LEN + 4];
        snprintf(trunc, sizeof trunc, "%s%s", name, is_dir ? "/" : "");
        eh_utf8_fit_width(trunc, sizeof trunc, w - 64);
        DrawString(32, row_y + (EH_FOLDER_ROW_H - 28) / 2 - 2, trunc);
    }
}
...
browser_navigate(const char *name)
{
    if (strcmp(name, "..") == 0) {
        ...
        char   next[sizeof eh_g_browse_path];
        size_t plen = strlen(eh_g_browse_path);
        size_t nlen = strlen(name);
        if (plen + 1 + nlen >= sizeof next)
            return; /* path already at the cap; cannot descend */
        memcpy(next, eh_g_browse_path, plen);
        next[plen] = '/';
        memcpy(next + plen + 1, name, nlen);
        next[plen + 1 + nlen] = '\0';
        memcpy(eh_g_browse_path, next, sizeof eh_g_browse_path);
    }
    eh_g_browse_scroll = 0;
    browser_load();
    if (s_bmode == BR_MODE_PICKER) {
        ...
        eh_draw_top_bar();
        PartialUpdate(0, 0, ScreenWidth(), EH_TOP_BAR_H);
        eh_draw_browse();
    }
    eh_flush_content();
}
...
browser_open_book(const char *path)
{
    ...
    if (tlen > EH_MAX_TITLE_LEN - 1)
        tlen = EH_MAX_TITLE_LEN - 1;
    ...
    eh_LOG("[bookshelf] browse: opening %s\n", path);
    eh_launch_reader(&b);
}
...
void
eh_browse_start(const char *dir)
{
    s_bmode = BR_MODE_BROWSER;
    snprintf(eh_g_browse_path, sizeof eh_g_browse_path, "%s", dir);
    eh_g_browse_scroll = 0;
    eh_g_browser_drag = 0;
...
void
eh_browse_page(int dir)
{
    eh_g_browse_scroll += dir * browser_rows_visible();
    eh_draw_browse();
    eh_flush_content();
}
...
{
    int w = ScreenWidth();
    int top = EH_TOP_BAR_H + EH_TOP_BAR_PAD;
    int bottom = eh_content_bottom();
    int rows = browser_rows_visible();
    ...
    for (int i = 0; i < rows; i++) {
        int idx = eh_g_browse_scroll + i;
        int row_y = top + 8 + i * EH_FOLDER_ROW_H;
    ...
    if (g_browse_count == 0) {
        ...
        if (f != NULL) {
            SetFont(f, DGRAY);
            DrawString(32, top + 32, eh_i18n("folder.empty"));
            CloseFont(f);
        }
    }
    eh_draw_scroll_buttons(eh_g_browse_scroll > 0, eh_g_browse_scroll < max_scroll);
}

int
eh_on_tap_browse(int x, int y)
{
    ...
    if (y < EH_TOP_BAR_H + EH_TOP_BAR_PAD)
        return 1;
    int idx = (y - (EH_TOP_BAR_H + EH_TOP_BAR_PAD + 8)) / EH_FOLDER_ROW_H + eh_g_browse_scroll;
...
folder_buttons(int *sel_x, int *sel_y, int *sel_w, int *sel_h)
{
    int w = ScreenWidth();
    *sel_w = (w - 64 - 16) / 2;
    *sel_h = EH_FOLDER_BTN_H;
    *sel_x = 32;
    *sel_y = eh_content_bottom() - EH_FOLDER_BTN_H - EH_FOLDER_BTN_PAD;
}
...
folder_commit(void)
{
    snprintf(eh_g_settings_dl_dir, sizeof eh_g_settings_dl_dir, "%s", eh_g_browse_path);
    eh_LOG("[bookshelf] folder: selected %s\n", eh_g_browse_path);
...
eh_folder_open(void)
{
    ...
    eh_g_state.overlay = EH_OV_FOLDER;
    eh_draw_overlay_folder();
    eh_flush_content();
}
...
overlay_header(int w)
{
    ...
    if (tf != NULL) {
        SetFont(tf, BLACK);
        DrawString(32, 24, eh_i18n("settings.dl_dir"));
        CloseFont(tf);
    }
    ...
    if (pf != NULL) {
        SetFont(pf, DGRAY);
        char trunc[256];
        eh_user_path_display(eh_g_browse_path, trunc, sizeof trunc);
        eh_utf8_fit_width(trunc, sizeof trunc, w - 64);
...
overlay_rows(void)
{
    ...
    int shown = 0;
    for (int i = 0; i < rows; i++) {
        int row_y = EH_FOLDER_LIST_TOP + i * EH_FOLDER_ROW_H;
        const char *name = NULL;
        if (up_row && i == 0) {
            name = "..";
        } else {
            int idx = eh_g_browse_scroll + i - up_row;
            if (idx >= 0 && idx < g_browse_count)
    ...
    if (shown == 0) {
        ...
        if (f != NULL) {
            SetFont(f, DGRAY);
            DrawString(32, EH_FOLDER_LIST_TOP + 24, eh_i18n("folder.empty"));
            CloseFont(f);
        }
    }
}
...
{
    int sy0 = eh_content_bottom() - EH_FOLDER_BTN_H - EH_FOLDER_BTN_PAD - EH_SCROLL_BTN_H;
    int dir = eh_hit_scroll_button_at(x, y, sy0);
    if (dir != 0) {
        int rows = browser_rows_visible();
        eh_g_browse_scroll += dir * rows;
        if (eh_g_browse_scroll < 0)
            eh_g_browse_scroll = 0;
        eh_draw_overlay_folder();
        eh_flush_content();
...
overlay_tap_row(int y)
{
    int rows = browser_rows_visible();
    int up_row = browser_can_go_up() ? 1 : 0;
    int idx = (y - EH_FOLDER_LIST_TOP) / EH_FOLDER_ROW_H;
    if (idx < 0 || idx >= rows)
        return 0;
    if (up_row && idx == 0) {
        browser_navigate("..");
    ...
    int dir_idx = eh_g_browse_scroll + idx - up_row;
...
eh_draw_overlay_folder(void)
{
    ...
    overlay_header(w);
    DrawLine(0, EH_FOLDER_LIST_TOP - 12, w, EH_FOLDER_LIST_TOP - 12, BLACK);
    ...
    if (f != NULL) {
        SetFont(f, WHITE);
        int tw = StringWidth(eh_i18n("folder.select"));
        DrawString(sx + (sw - tw) / 2, sy + (sh - 12 - 32) / 2, eh_i18n("folder.select"));
    }
    int bx = sx + sw + 16;
    ...
    if (f != NULL) {
        SetFont(f, BLACK);
        int tw = StringWidth(eh_i18n("settings.back"));
        DrawString(bx + (sw - tw) / 2, sy + (sh - 12 - 32) / 2, eh_i18n("settings.back"));
        CloseFont(f);
    }

    /* Corner scroll buttons, raised above the Select/Back band. */
    int sy0 = eh_content_bottom() - EH_FOLDER_BTN_H - EH_FOLDER_BTN_PAD - EH_SCROLL_BTN_H;
    eh_draw_scroll_buttons_at(eh_g_browse_scroll > 0, eh_g_browse_scroll < max_scroll, sy0);
}

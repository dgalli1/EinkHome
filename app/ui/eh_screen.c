/* eh_screen.c — part of the bookshelf app (see eh_core.h) */
 ...
 * splits a multibyte UTF-8 sequence.  Moved from eh_browser.c and
...
void
eh_utf8_fit_width(char *s, size_t cap, int maxw)
{
    if (cap < 1)
        return;
    size_t len = strlen(s);
...
void
eh_draw_text_centered(ifont *f, int cx, int cy, const char *text, int color)
{
    if (f == NULL)
        return;
    SetFont(f, color);
 ...
 * tab's magnifier). */
void
eh_draw_circle_outline(int cx, int cy, int r, int col)
{
    int px = 0, py = 0;
    for (int s = 0; s <= 16; s++) {
 ...
 * Shared by the log viewer (eh_logview.c) and the licenses viewer
 * (eh_licenses.c); the word-wrap placers there stay local because they
 * operate on their own row types (BsLogRow vs BsLicRow with its `blank`
 * gap rows), so only the width primitive is deduplicated. */
int
eh_span_width(const char *p, int len)
{
    char tmp[1024];
...
void
eh_draw_system_strip(void)
{
    int w = ScreenWidth();
    int h = eh_g_state.panel_h;
 ...
 * PB backend (eh_plat_stamp_panel). */
void
eh_stamp_panel(void)
{
    eh_plat_stamp_panel(eh_g_self_panel);
}
 ...
 * leaves it stale. */
void
eh_flush_content(void)
{
    PartialUpdate(0, 0, ScreenWidth(), eh_content_bottom());
    PartialUpdate(0, eh_content_bottom() - EH_PAGER_H, ScreenWidth(), EH_PAGER_H);
}
...
eh_draw_shelf_nofb(void)
{
    ...
    FillArea(0, 0, ScreenWidth(), eh_content_bottom(), WHITE);
    eh_draw_top_bar();
    if (eh_g_state.tab == EH_TAB_SEARCH)
        eh_draw_search_tab();
...
void
eh_draw_scroll_buttons_at(int up_ok, int down_ok, int y0)
{
    if (!up_ok && !down_ok)
        return;
    int w = ScreenWidth();

    FillArea(0, y0, EH_SCROLL_BTN_W, EH_SCROLL_BTN_H, WHITE);
    DrawRect(0, y0, EH_SCROLL_BTN_W, EH_SCROLL_BTN_H, up_ok ? BLACK : LGRAY);
    int col = up_ok ? BLACK : LGRAY;
    int cx = EH_SCROLL_BTN_W / 2;
    int cy = y0 + EH_SCROLL_BTN_H / 2;
    DrawLine(cx - 24, cy + 14, cx, cy - 14, col);
    DrawLine(cx + 24, cy + 14, cx, cy - 14, col);

    int x2 = w - EH_SCROLL_BTN_W;
    FillArea(x2, y0, EH_SCROLL_BTN_W, EH_SCROLL_BTN_H, WHITE);
    DrawRect(x2, y0, EH_SCROLL_BTN_W, EH_SCROLL_BTN_H, down_ok ? BLACK : LGRAY);
    col = down_ok ? BLACK : LGRAY;
    cx = x2 + EH_SCROLL_BTN_W / 2;
    DrawLine(cx - 24, cy - 14, cx, cy + 14, col);
    DrawLine(cx + 24, cy - 14, cx, cy + 14, col);
}

void
eh_draw_scroll_buttons(int up_ok, int down_ok)
{
    eh_draw_scroll_buttons_at(up_ok, down_ok, eh_content_bottom() - EH_SCROLL_BTN_H);
}

/* Hit test for the corner scroll buttons: -1 = up (bottom-left),
 * +1 = down (bottom-right), 0 = neither. */
int
eh_hit_scroll_button_at(int x, int y, int y0)
{
    int w = ScreenWidth();
    if (y < y0 || y >= y0 + EH_SCROLL_BTN_H)
        return 0;
    if (x >= 0 && x < EH_SCROLL_BTN_W)
        return -1;
    if (x >= w - EH_SCROLL_BTN_W && x < w)
        return +1;
    return 0;
}

int
eh_hit_scroll_button(int x, int y)
{
    return eh_hit_scroll_button_at(x, y, eh_content_bottom() - EH_SCROLL_BTN_H);
}

/* ── full-screen overlay header ─────────────────────────────────────── */

/* Back-button touch box in the shared overlay header: used by the draw
 * path (eh_draw_overlay_header) and every overlay's tap hit-test so
 * the tappable region always matches the painted chevron. */
void
eh_overlay_back_rect(int *bx, int *by, int *bw, int *bh)
{
    *bx = EH_OVERLAY_BACK_X;
    *by = EH_OVERLAY_BACK_Y;
    *bw = EH_OVERLAY_BACK_W;
    *bh = EH_OVERLAY_BACK_H;
}
 ...
 * EH_OVERLAY_* in eh_core.h) keeps the three headers pixel-identical;
 * before the share, the settings and log headers had drifted to
 * different heights and offsets. */
void
eh_draw_overlay_header(const char *title)
{
    int w = ScreenWidth();
    FillArea(0, 0, w, EH_OVERLAY_HEADER_H, WHITE);
    DrawLine(0, EH_OVERLAY_HEADER_H - 1, w, EH_OVERLAY_HEADER_H - 1, BLACK);

    int bx, by, bw, bh;
    eh_overlay_back_rect(&bx, &by, &bw, &bh);
    eh_draw_back_icon(bx + bw / 2, by + bh / 2, BLACK);

    ifont *tf = OpenFont(DEFAULTFONTB, 36, 0);
    if (tf != NULL) {
        SetFont(tf, BLACK);
        int tw = StringWidth(title);
        DrawString((w - tw) / 2, (EH_OVERLAY_HEADER_H - 36) / 2, title);
        CloseFont(tf);
    }
}

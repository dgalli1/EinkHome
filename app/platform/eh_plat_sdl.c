/* eh_plat_sdl.c — native PC backend over SDL2 (see app/platform/eh_plat.h).
 *
 ...
 * drives.  It implements both the eh_plat_* backend functions AND the
 ...
 *    press F11 to cycle through them at runtime, or set the EH_RES launch
 *    flag ("EH_RES=1404x1872 ./scripts/run-visible-sdl.sh") to start on
 ...
 * EH_PLATFORM_SDL so app/platform/eh_plat.h selects the compat headers.
 */
 ...
 * EH_RES launch flag.  g_sw/g_sh are the *current* logical canvas size;
 * the window scales it to fit (EH_SCALE) and SDL maps pointer
...
static double  g_scale = 0.6;                            /* window scale (EH_SCALE) */
...
QueryNetwork(void)
{
    /* Test hook: EH_OFFLINE=1 reports "no active connection" so the app
     * behaves like a real device with WiFi off — it skips the boot
     * auto-sync and skips remote cover fetches (see eh_main.c
     * init_sync_tick and eh_grid.c cover_tick).  This is how the SDL
     * e2e suite simulates no internet (the emulator gets the same from
     * the firmware's real QueryNetwork). */
    if (getenv("EH_OFFLINE") != NULL)
        return 0;
 ...
 * grid) is what keeps a popup on top of the covers.  eh_redraw_shelf
 ...
 * eh_plat_cover_flush() (called by the app between the body and the
...
FullUpdate(void)
{
    ...
    sdl_merge_covers();
    /* Debug: EH_DUMP_FRAME=path writes one PPM of the canvas (for
     * headless visual verification — e.g. in CI / docs).  The very
     * first FullUpdate fires before covers load; use EH_DUMP_AFTER_MS
     * to dump a settled frame instead (see the main loop). */
    const char *dump = getenv("EH_DUMP_FRAME");
    if (dump && g_dump_pending == 0) {
        snprintf(g_dump_path, sizeof g_dump_path, "%s", dump);
        g_dump_pending = 1;
        const char *after = getenv("EH_DUMP_AFTER_MS");
...
sdl_resolve_initial_resolution(void)
{
    ...
    for (int i = 0; i < PC_RES_COUNT; i++) {
        ...
        fprintf(stderr, "[pc] EH_RES=%s -> %dx%d\n", e, g_sw, g_sh);
        return;
    }
    fprintf(stderr, "[pc] EH_RES=%s: no supported match, keeping %dx%d\n",
            e, g_sw, g_sh);
}
 ...
 * desktop height (~92%).  EH_SCALE still governs whenever it fits. */
static void
...
sdl_create_window_state(void)
{
     ...
     * fits a 1080p desktop; EH_SCALE overrides (e.g. 1.2 = larger). */
    const char *scale_env = getenv("EH_SCALE");
...
/* Debug: EH_AUTO_LAUNCHER=1 opens the app launcher right after boot
 * (headless verification of the .desktop-backed launcher listing).
 * eh_launcher_open_set() builds the list (through the platform seam
 * → eh_plat_launcher_build → .desktop parser) and draws it. */
static void
...
eh_plat_boot(int (*on_event)(int, int, int))
{
    ...
    if (getenv("EH_AUTO_LAUNCHER") != NULL) {
        sdl_boot_auto_launcher();
    }

    while (g_running) {
        Uint32 now_ms = SDL_GetTicks();
        /* Delayed frame dump: dump once EH_DUMP_AFTER_MS have elapsed
...
eh_plat_panel_height(int *self_panel)
{
     ...
     * treats the whole window as content (eh_content_bottom() =
...
{
    eh_LOG("[bookshelf] model=PC fw=%s\n", GetSoftwareVersion());
}
...
parse_desktop_exec(const char *exec, char *out_cmd, size_t cmd_cap,
                   char params[][EH_LAUNCHER_PARAM_LEN], int *nparams)
{
    ...
    out_cmd[ci] = '\0';
    *nparams = 0;
    char tok[EH_LAUNCHER_PARAM_LEN];
    while (*p) {
        ...
        if (tok[0] && *nparams < EH_LAUNCHER_MAX_PARAMS) {
            snprintf(params[(*nparams)++], EH_LAUNCHER_PARAM_LEN, "%s", tok);
        }
    }
}
...
desktop_file_to_item(const char *path, BsLauncherItem *it, int *n)
{
    ...
    if (!f) return;
    char line[512];
    char name[96] = "", exec[EH_MAX_PATH_LEN] = "", icon[64] = "";
 ...
 *
 * Compiled in ONLY when EH_ENABLE_TEST_IPC is defined (the test build —
 ...
 *
 * Socket path: $EH_SOCKET, else /tmp/bookshelf-<pid>.sock.  A blank or
 * "off" EH_SOCKET disables the control plane.  Because the socket is
 * per-process, parallel test runs (each with its own pid / EH_SOCKET)
 * never collide. */

#ifdef EH_ENABLE_TEST_IPC /* control socket: test builds only */
...
ipc_query_group(const char *cmd, int n, const char *a)
{
    if (strcmp(cmd, "key") == 0 && n >= 2) {
        ...
        snprintf(s, sizeof s, "state=%d:%d:%d\n", eh_g_state.overlay,
                 eh_g_state.tab, eh_g_state.page);
        ipc_reply("%s", s);
    } else {
        ipc_reply("err unknown cmd\n", NULL);
    }
...
ipc_init(void)
{
    const char *env = getenv("EH_SOCKET");

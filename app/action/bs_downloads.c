/* bs_downloads.c — part of the bookshelf app (see bs_core.h) */
...
{
    DIR *d = opendir(bs_g_downloads_dir);
    if (d == NULL)
        return;
    struct dirent *e;
    ...
    while ((e = readdir(d)) != NULL && seen < 8192) {
        ...
        snprintf(path, sizeof path, "%s/%s", bs_g_downloads_dir, e->d_name);
        if (unlink(path) == 0)
            removed++;
    }
    closedir(d);
    bs_LOG("[bookshelf] stale .part sweep removed=%d\n", removed);
}
...
void
bs_book_local_path(const BsBook *b, char *out, size_t cap)
{
    if (b->filename[0] != '\0' && strcmp(b->filename, ".") != 0 && strcmp(b->filename, "..") != 0) {
        char   sanitized[BS_MAX_PATH_LEN];
        size_t n = 0;
        ...
        sanitized[n] = '\0';
        if (n > 0) {
            snprintf(out, cap, "%s/%s", bs_g_downloads_dir, sanitized);
            return;
        }
    }
    if (b->ext[0])
        snprintf(out, cap, "%s/%s.%s", bs_g_downloads_dir, b->id, b->ext);
    else
        snprintf(out, cap, "%s/%s", bs_g_downloads_dir, b->id);
}
...
void
bs_book_existing_path(const BsBook *b, char *out, size_t cap)
{
...
#define BS_DL_FLAG_PAGES_PER_TICK 8 /* probe pages (64 books each) per slice */
...
dl_flag_arm(void)
{
    ...
    if (s == NULL) {
        bs_LOG("[bookshelf] refresh_downloaded_flags: scan alloc failed\n");
...
dl_flag_finish(void)
{
    ...
    free(s);
    g_dl_flag_scan = NULL;
    bs_LOG("[bookshelf] refresh_downloaded_flags: changed=%d\n", changed);
}

/* Arm the boot flag scan (called by bs_main's bootslice timer). */
void
bs_refresh_downloaded_flags_boot_start(void)
{
    dl_flag_arm();
}
...
int
bs_refresh_downloaded_flags_boot_step(void)
{
    dl_flag_arm(); /* auto-arm on the first step */
    BsDlFlagScan *s = g_dl_flag_scan;
    if (s == NULL)
        return 1; /* couldn't arm: treat as done */
    for (int page = 0; page < BS_DL_FLAG_PAGES_PER_TICK; page++) {
        int got = bs_store_next_dl_probes(s->probes, 64, &s->rowid);
        if (got <= 0) {
            dl_flag_finish();
            return 1;
        }
        ...
        for (int i = 0; i < got; i++) {
            ...
            if (dl != p->downloaded) {
                bs_store_set_downloaded(p->id, dl, dl ? path : "");
                s->changed++;
            }
        }
...
void
bs_refresh_downloaded_flags(void)
{
    dl_flag_arm();
    while (!bs_refresh_downloaded_flags_boot_step())
        ;
}

/* Find a download-queue entry by id (NULL if absent). */
BsDownloadItem *
bs_find_download(const char *id)
{
    for (int i = 0; i < bs_g_download_count; i++)
        if (strcmp(bs_g_downloads[i].id, id) == 0)
            return &bs_g_downloads[i];
    return NULL;
}
...
clear_finished_downloads(void)
{
    int w = 0;
    for (int i = 0; i < bs_g_download_count; i++) {
        if (bs_g_downloads[i].state == 2 || bs_g_downloads[i].state == 3)
            continue;
        if (w != i)
            bs_g_downloads[w] = bs_g_downloads[i];
        w++;
    }
    bs_g_download_count = w;
}
 ...
 * right away. */
void
bs_enqueue_download(const BsBook *b)
{
    BsDownloadItem *d = bs_find_download(b->id);
    if (d != NULL && (bs_g_dl_batch_active || d->state != 3))
        return;
    ...
    if (bs_g_download_count >= BS_MAX_DOWNLOADS)
        return;
    BsDownloadItem *n = &bs_g_downloads[bs_g_download_count++];
...
prune_finished_download(void)
{
    ...
    if (best < 0)
        return 0;
    for (int i = best; i + 1 < bs_g_download_count; i++)
        bs_g_downloads[i] = bs_g_downloads[i + 1];
    bs_g_download_count--;
    return 1;
}
...
dl_fetch(BsJob *job)
{
    ...
    if (data != NULL && rsize > 0) {
        if (atomic_load_explicit(&job->cancel, memory_order_acquire)) {
            ...
            if (f != NULL) {
                ...
                if (!werr && rename(tmp, a->path) == 0) {
                    ok = 1;
                    bs_LOG("[bookshelf] download_book_file OK id=%s path=%s bytes=%d\n",
                        a->id, a->path, rsize);
                } else {
                    bs_LOG("[bookshelf] download_book_file write/rename FAILED "
                        "id=%s path=%s errno=%d\n",
                        a->id, a->path, errno);
                    unlink(tmp); /* never leave the .part behind */
                }
            } else {
                bs_LOG("[bookshelf] download_book_file fopen FAILED id=%s path=%s errno=%d\n",
                    a->id, a->path, errno);
            }
        }
        free(data);
        ...
        if (data != NULL)
            free(data);
        bs_LOG("[bookshelf] download_book_file FAILED id=%s url=%s rsize=%d errno=%d\n",
...
bs_launch_reader(BsBook *b)
{
    ...
    if (bs_g_state.reader_pref > 0 && bs_g_state.reader_pref <= bs_g_reader_count)
        reader_path = bs_g_readers[bs_g_state.reader_pref - 1].path;
    if (reader_path != NULL && access(reader_path, X_OK) == 0 &&
        strcmp(reader_path, BS_READER_STD_PATH) != 0) {
        const char *rbase = strrchr(reader_path, '/');
        rbase = rbase ? rbase + 1 : reader_path;
        char *args[3] = {(char *)reader_path, path, NULL};
        bs_LOG("[bookshelf] launching reader app=%s path=%s reader_pref=%d\n",
            rbase,
            path,
    ...
    bs_LOG("[bookshelf] launching reader via OpenBook path=%s reader_pref=%d\n",
        path,
        bs_g_state.reader_pref);
...
bs_book_press_action(BsBook *b)
{
    ...
    if (!b->downloaded) {
        ...
        bs_g_state.dl_popup_auto_open = 1;
        snprintf(bs_g_state.dl_popup_book_id, sizeof bs_g_state.dl_popup_book_id, "%s", b->id);
        bs_enqueue_download(b);
        bs_redraw_shelf(); /* draws the popup on top */
        return;
    }
    bs_launch_reader(b);
}
...
batch_failed_id(const char *id)
{
    return bsearch(id, g_dl_batch_failed_ids,
                   (size_t)g_dl_batch_failed_count, BS_MAX_ID_LEN,
...
batch_note_failed(const char *id)
{
    if (g_dl_batch_failed_count >= g_dl_batch_failed_cap) {
        ...
        char  *nids = realloc(g_dl_batch_failed_ids, (size_t)newcap * BS_MAX_ID_LEN);
        if (nids == NULL)
    ...
    while (lo < hi) {
        int mid = (lo + hi) / 2;
        if (strcmp(g_dl_batch_failed_ids + (size_t)mid * BS_MAX_ID_LEN, id) < 0)
            lo = mid + 1;
        else
            hi = mid;
    }
    char *dst = g_dl_batch_failed_ids + (size_t)lo * BS_MAX_ID_LEN;
    size_t tail = (size_t)(g_dl_batch_failed_count - lo) * BS_MAX_ID_LEN;
    if (tail > 0)
        memmove(dst + BS_MAX_ID_LEN, dst, tail);
    snprintf(dst, BS_MAX_ID_LEN, "%s", id);
...
batch_enqueue_slice(int *got)
{
    ...
    *got = bs_store_next_undownloaded(ids, 64);
    int enq = 0;
    for (int i = 0; i < *got; i++) {
        if (bs_find_download(ids[i]) != NULL)
            continue;
...
void
bs_download_all_start(void)
{
    int total = bs_store_count_undownloaded();
    if (total == 0) {
        bs_LOG("[bookshelf] download-all nothing to download\n");
        return;
    }
    ...
    bs_g_dl_batch_active = 1;
    bs_g_dl_batch_total = total;
    bs_g_dl_batch_done = 0;
    bs_g_dl_batch_failed = 0;
    int got = 0;
    ...
    bs_redraw_shelf();
    bs_LOG("[bookshelf] download-all queued=%d\n", bs_g_dl_batch_total);
}
...
dl_start_next(void)
{
    ...
    int attempted = 0;
    for (int i = 0; i < bs_g_download_count; i++) {
        BsDownloadItem *target = &bs_g_downloads[i];
        if (target->state != 0)
            continue;
        attempted = 1;
        target->state = 1;
        ...
        BsBook b;
        if (!bs_store_get_book(target->id, &b)) {
            target->state = 3;
            continue;
        }
...
dl_job_done(BsJob *job)
{
    ...
    BsDownloadItem *d = bs_find_download(a->id);
    if (d != NULL && d->gen == a->gen) {
        d->state = ok ? 2 : 3;
        if (ok)
            bs_store_set_downloaded(d->id, 1, a->path);
        if (bs_g_dl_batch_active) {
        ...
        if (!bs_g_state.dl_popup)
            bs_draw_top_bar();
        bs_sync_set_active(bs_downloads_pending() > 0 || bs_g_dl_batch_active);
    } else if (d != NULL) {
        ...
        bs_LOG("[bookshelf] stale download job settle dropped id=%s gen=%u entry_gen=%u\n",
            a->id,
            a->gen,
            d->gen);
    }
    ...
    bs_dl_progress_metrics(&dtotal, &ddone, &dfailed, &dactive);
    bs_LOG("[bookshelf] dl_progress done=%d failed=%d total=%d active=%d\n",
...
dl_advance(void)
{
    ...
    if (bs_g_state.dl_popup)
        ...
        bs_draw_top_bar(); /* refresh the pending-count badge in top bar */
}
...
void
bs_cancel_downloads(void)
{
    bs_LOG("[bookshelf] cancel_downloads batch=%d in_flight=%p\n",
        bs_g_dl_batch_active, (void *)g_dl_inflight);
    bs_g_dl_batch_active = 0;
    bs_g_dl_batch_total = 0;
    bs_g_dl_batch_done = 0;
    bs_g_dl_batch_failed = 0;
...
void
bs_context_geom(int *px, int *py, int *pw, int *ph, int n_items)
{
    int w = ScreenWidth();
    int h = ScreenHeight();
    *pw = w * 3 / 4;
    *ph = BS_CTX_TITLE_H + n_items * BS_CTX_ITEM_H + BS_CTX_PAD;
    *px = (w - *pw) / 2;
    *py = (h - *ph) / 2;
}

int
bs_context_item_count(void)
{
    /* A book offers Open + Download + Delete; a series card offers
     * Download all + Delete series. */
    return bs_g_state.ctx_is_series ? 2 : 3;
}

/* Draw the long-press context menu over a dimmed shelf. */
void
bs_draw_context_menu(void)
{
    int w = ScreenWidth();
    /* Dim mask over the whole app content area (panel band stays). */
    for (int yy = 0; yy < bs_content_bottom(); yy += 2)
        DrawLine(0, yy, w, yy, LGRAY);

    int n = bs_context_item_count();
    int px, py, pw, ph;
    bs_context_geom(&px, &py, &pw, &ph, n);
    ...
    static char title_buf[BS_MAX_TITLE_LEN];
    const char *title;
    if (bs_g_state.ctx_is_series) {
        bs_store_series_name(bs_g_state.ctx_series_id, title_buf, sizeof title_buf);
        title = title_buf[0] != '\0' ? title_buf : "Series";
    } else {
        BsBook tmp;
        title_buf[0] = '\0';
        if (bs_store_get_book(bs_g_state.ctx_book_id, &tmp))
    ...
    if (tf != NULL) {
        ...
        DrawString(px + BS_CTX_PAD, py + (BS_CTX_TITLE_H - 28) / 2 - 2, trunc);
        CloseFont(tf);
    }
    ...
    if (bs_g_state.ctx_is_series) {
        ...
        labels[0] = bs_i18n("ctx.open");
        labels[1] = bs_i18n("ctx.download");
        labels[2] = bs_i18n("ctx.delete");
    }
    ...
    SetFont(f, BLACK);
    for (int i = 0; i < n; i++) {
        int iy = py + BS_CTX_TITLE_H + i * BS_CTX_ITEM_H;
        DrawString(px + BS_CTX_PAD, iy + (BS_CTX_ITEM_H - 30) / 2 - 2, labels[i]);
        if (i + 1 < n)
            DrawLine(
                px + BS_CTX_PAD, iy + BS_CTX_ITEM_H - 1, px + pw - BS_CTX_PAD, iy + BS_CTX_ITEM_H - 1, LGRAY);
    }
    CloseFont(f);
}

void
bs_close_context(void)
{
...
void
bs_open_context_for_tile(int vi)
{
    BsTileRow tr;
    if (!bs_view_fetch_row(vi, &tr))
        return;
    bs_g_state.overlay = BS_OV_CTX;
    bs_g_state.ctx_is_series = tr.is_series;
    if (tr.is_series) {
        snprintf(bs_g_state.ctx_series_id, sizeof bs_g_state.ctx_series_id, "%s", tr.series_id);
        bs_g_state.ctx_book_id[0] = '\0';
    } else {
        snprintf(bs_g_state.ctx_book_id, sizeof bs_g_state.ctx_book_id, "%s", tr.book.id);
        bs_g_state.ctx_series_id[0] = '\0';
    }
    bs_draw_context_menu();
    bs_flush_content();
    bs_LOG("[bookshelf] context menu open series=%d vi=%d\n", tr.is_series, vi);
}

/* Long-press timer fired with the finger still down: open the menu. */
void
bs_longpress_tick(void *ctx)
{
    (void)ctx;
    if (!bs_g_lp_armed || bs_g_lp_vi < 0)
        return;
    bs_g_lp_armed = 0;
    int vi = bs_g_lp_vi;
    bs_g_lp_vi = -1;
    bs_g_ctx_suppress_up = 1;
    bs_open_context_for_tile(vi);
}

/* Handle a tap while the context menu is open. */
void
bs_on_tap_context(int x, int y)
{
    int n = bs_context_item_count();
    int px, py, pw, ph;
    bs_context_geom(&px, &py, &pw, &ph, n);
    if (x < px || x >= px + pw || y < py + BS_CTX_TITLE_H || y >= py + ph) {
        bs_close_context();
        return;
    }
    int  item = (y - (py + BS_CTX_TITLE_H)) / BS_CTX_ITEM_H;
    int  is_series = bs_g_state.ctx_is_series;
    char series_id[BS_MAX_ID_LEN];
    snprintf(series_id, sizeof series_id, "%s", bs_g_state.ctx_series_id);
    bs_g_state.overlay = BS_OV_NONE;

    if (is_series) {
        if (item == 0)
            bs_download_series(series_id);
        else if (item == 1)
            bs_delete_series(series_id);
    } else {
        BsBook b;
        if (bs_store_get_book(bs_g_state.ctx_book_id, &b)) {
            if (item == 0) {
                /* Open works exactly like a single tap: download if
                 * needed (with the progress popup), then launch. */
                bs_book_press_action(&b);
            } else if (item == 1) {
                bs_g_state.dl_popup = 1;
                bs_g_state.dl_popup_auto_open = 0;
                bs_enqueue_download(&b);
            } else if (item == 2) {
                bs_store_delete_book_file(b.id);
            }
        }
    }
    bs_redraw_shelf();

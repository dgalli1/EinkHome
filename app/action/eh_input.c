/* eh_input.c — part of the bookshelf app (see eh_core.h) */
...
eh_hit_top_bar(int x, int y)
{
     ...
     * removed). */
    if (x >= EH_TOP_BTN_PAD && x < EH_TOP_BTN_PAD + EH_TOP_BTN_SIZE)
        return 1;
    ...
    if (x >= EH_SOURCE_BTN_X && x < EH_SOURCE_BTN_X + eh_source_btn_w())
        return 6;
    return eh_hit_top_bar_right(x, w);
}

/* 1 when (x, y) is inside the search input row on the Search page. */
int
eh_hit_search_input(int x, int y)
{
    int row_top = EH_TOP_BAR_H + EH_TOP_BAR_PAD;
    int row_bot = row_top + EH_SEARCH_ROW_H;
    if (y < row_top || y >= row_bot)
        return -1;
    int w = ScreenWidth();
    int tx = 16, tw = w - 32; /* full-width bar (see draw_search_tab) */
    int ty = row_top + 10;
    int th = EH_SEARCH_ROW_H - 20;
    if (x < tx || x >= tx + tw)
        return -1;
    if (y < ty || y >= ty + th)
        return -1;
...
eh_hit_history(int x, int y)
{
    ...
    int y0 = top + EH_SEARCH_ROW_H;
    if (y < y0)
        return -1;
    int ps = eh_history_pagesize();
    if (ps < 1)
        ps = 1;
    int rel = (y - y0) / EH_SEARCH_HISTORY_ROW_H;
    if (rel >= ps)
        return -1;
    int idx = eh_g_state.page * ps + rel;
    if (idx >= eh_store_search_count())
        return -1;
    return idx;
}
...
eh_hit_suggestion(int x, int y)
{
    ...
    if (y < y_top || y >= y_bot)
        return -1;
    int drawn = (y_bot - y_top) / EH_SEARCH_HISTORY_ROW_H;
    int rel = (y - y_top) / EH_SEARCH_HISTORY_ROW_H;
    if (rel >= drawn || rel >= eh_g_nsuggest)
        return -1;
    return rel;
}

int
eh_hit_thumbnail(int x, int y)
{
    int top, bot, cell_w, cell_h;
    eh_grid_geom(&top, &bot, &cell_w, &cell_h);
    ...
    int last = page_start + eh_view_pagesize();
    if (last > eh_g_view_total)
        last = eh_g_view_total;
...
static int
eh_hit_pager_prev(int x)
{
    /* < prev — 96px wide starting at x=12 */
    if (eh_g_state.page > 0 && x >= 12 && x < 12 + 96)
        return -1;
    /* << first page — next 96px slot */
    if (eh_g_state.page > 0 && x >= 116 && x < 116 + 96)
        return -3;
    return 0;
}
...
static int
eh_hit_pager_next(int x, int w, int pages)
{
    /* >> last page — 96px slot left of the next button */
    if (eh_g_state.page + 1 < pages && x >= w - 212 && x < w - 116)
        return -4;
    /* > next — 96px wide ending at x=w-12 */
    if (eh_g_state.page + 1 < pages && x >= w - 108 && x < w - 12)
        return -2;
    return 0;
}
...
int
eh_hit_pager(int x, int y)
{
    int y0 = eh_content_bottom() - EH_PAGER_H;
    if (y < y0 || y >= y0 + EH_PAGER_H)
        return 0;
    int w = ScreenWidth();
    int pages = eh_current_pages();
    int r = eh_hit_pager_prev(x);
    if (r != 0)
        return r;
    return eh_hit_pager_next(x, w, pages);
}

/* ── tap handlers ────────────────────────────────────────────────────── */

int
eh_on_tap_overlay_group(int x, int y)
{
    ...
    eh_g_group = opts[r];
    eh_g_drill_level = 0;
    for (int L = 0; L < EH_GROUP_MAX_LEVELS; L++)
        eh_g_drill_values[L][0] = '\0';
    eh_g_state.page = 0;
    eh_view_rebuild();
    eh_g_state.overlay = EH_OV_NONE;
    return 1;
...
int
eh_on_tap_overlay_sort(int x, int y)
{
    int w = ScreenWidth();
    int pw = w * 3 / 4;
    int ph = 72 + 4 * 96 + 24;
...
int
eh_on_tap_overlay_more(int x, int y)
{
    ...
    if (y >= y0 + EH_MORE_GROUP_IDX * EH_MORE_ITEM_H &&
        y < y0 + (EH_MORE_GROUP_IDX + 1) * EH_MORE_ITEM_H) {
        eh_g_state.overlay = EH_OV_GROUP;
        eh_draw_overlay_group();
        FullUpdate();
        return 1;
    }
    if (y >= y0 + EH_MORE_SORT_IDX * EH_MORE_ITEM_H &&
        y < y0 + (EH_MORE_SORT_IDX + 1) * EH_MORE_ITEM_H) {
    ...
    if (y >= y0 + EH_MORE_SETTINGS_IDX * EH_MORE_ITEM_H &&
        y < y0 + (EH_MORE_SETTINGS_IDX + 1) * EH_MORE_ITEM_H) {
        eh_g_state.overlay = EH_OV_SETTINGS;
    ...
    if (y >= y0 + EH_MORE_APPS_IDX * EH_MORE_ITEM_H &&
        y < y0 + (EH_MORE_APPS_IDX + 1) * EH_MORE_ITEM_H) {
        eh_g_state.overlay = EH_OV_NONE;
    ...
    if (y >= y0 + EH_MORE_DLALL_IDX * EH_MORE_ITEM_H &&
        y < y0 + (EH_MORE_DLALL_IDX + 1) * EH_MORE_ITEM_H) {
        eh_g_state.overlay = EH_OV_NONE;
        eh_download_all_start();
        return 1;
    }
    eh_g_state.overlay = EH_OV_NONE;
    return 0;
...
void
eh_settings_close(void)
{
    eh_g_state.overlay = EH_OV_NONE;
    eh_g_settings_edit = 0;
    eh_g_settings_dl_dir[0] = '\0';
    eh_redraw_shelf();
}
...
void
eh_settings_toggle_sysapp(void)
{
    int want = !eh_g_state.sys_app_on;
    int rc = want ? eh_sysapp_promote() : eh_sysapp_unpromote();
    if (rc != 0) {
        eh_LOG("[bookshelf] sysapp: %s failed (target dir %s)\n",
               want ? "promote" : "unpromote", eh_sysapp_dir());
        return; /* keep showing the previous state */
    }
    eh_g_state.sys_app_on = want;
    eh_draw_overlay_settings();
    /* Only the toggle row changed; refresh it instead of a full-screen
     * flash. */
    PartialUpdate(0, 4 * EH_SETTINGS_ROW_H + 112, ScreenWidth(),
                  EH_SETTINGS_ROW_H - 12);
    eh_LOG("[bookshelf] sysapp: %s — %s\n",
...
static int
eh_on_tap_settings_api_host(int y, int y_row)
{
    if (y >= y_row && y < y_row + EH_SETTINGS_ROW_H - 12) {
        eh_g_settings_edit = 1;
        snprintf(eh_g_settings_kb_buf, sizeof eh_g_settings_kb_buf, "%s", eh_g_state.api_base);
        eh_draw_overlay_settings();
        FullUpdate();
        OpenKeyboard(eh_i18n("settings.api_host"),
                     eh_g_settings_kb_buf,
                     sizeof eh_g_settings_kb_buf - 1,
                     0,
                     eh_settings_keyboard_handler);
        return 1;
    }
    return 0;
}
...
static int
eh_on_tap_settings_api_token(int y, int y_row)
{
    if (y >= y_row && y < y_row + EH_SETTINGS_ROW_H - 12) {
        eh_g_settings_edit = 2;
        snprintf(eh_g_settings_kb_buf, sizeof eh_g_settings_kb_buf, "%s", eh_g_state.api_token);
        eh_draw_overlay_settings();
        FullUpdate();
        OpenKeyboard(eh_i18n("settings.api_key"),
                     eh_g_settings_kb_buf,
                     sizeof eh_g_settings_kb_buf - 1,
                     0,
                     eh_settings_keyboard_handler);
...
static int
eh_on_tap_settings_reader(int y, int y_row)
{
    if (y >= y_row && y < y_row + EH_SETTINGS_ROW_H - 12) {
        /* Cycle Auto → reader[0] → reader[1] → … → Auto. */
        eh_g_state.reader_pref = (eh_g_state.reader_pref + 1) % (eh_g_reader_count + 1);
        eh_draw_overlay_settings();
        /* Only the reader row's value text changed; refresh just that
         * row instead of a full-screen flash. */
        PartialUpdate(32, y_row, ScreenWidth() - 64, EH_SETTINGS_ROW_H - 12);
        return 1;
    }
    return 0;
}
...
static int
eh_on_tap_settings_sysapp(int y, int y_row)
{
    if (y >= y_row && y < y_row + EH_SETTINGS_ROW_H - 12) {
        eh_settings_toggle_sysapp();
        return 1;
    }
    return 0;
}
...
static int
eh_on_tap_settings_apply(int y, int y_save)
{
    if (y >= y_save && y < y_save + EH_SETTINGS_BTN_H - 12) {
        eh_settings_apply();
        return 1;
    }
    return 0;
...
static int
eh_on_tap_settings_logs(int y, int y_logs)
{
    if (y >= y_logs && y < y_logs + EH_SETTINGS_BTN_H - 12) {
        ...
        eh_g_settings_edit = 0;
        eh_g_state.overlay = EH_OV_LOG;
        eh_g_state.log_scroll = -1; /* start at the tail */
        eh_draw_log_view();
        FullUpdate();
        return 1;
    }
...
static int
eh_on_tap_settings_licenses(int y, int y_lic)
{
    if (y >= y_lic && y < y_lic + EH_SETTINGS_BTN_H - 12) {
        ...
        eh_g_settings_edit = 0;
        eh_g_state.overlay = EH_OV_LICENSES;
        eh_g_state.lic_sel = -1; /* start on the entry list */
...
void
eh_on_tap_overlay_settings(int x, int y)
{
    /* Header Back chevron (rows span the full content width; only the
     * button rect uses x). */
    int bx, by, bw, bh;
    eh_overlay_back_rect(&bx, &by, &bw, &bh);
    ...
    int y_row1 = 112;
    int y_row2 = y_row1 + EH_SETTINGS_ROW_H;
    int y_row3 = y_row2 + EH_SETTINGS_ROW_H;
    int y_row4 = y_row3 + EH_SETTINGS_ROW_H;
    int y_row5 = y_row4 + EH_SETTINGS_ROW_H;
    int y_save = y_row5 + EH_SETTINGS_ROW_H + 24;
    int y_logs = y_save + EH_SETTINGS_BTN_H;

    if (eh_on_tap_settings_api_host(y, y_row1))
        return;
    if (eh_on_tap_settings_api_token(y, y_row2))
        return;
    if (eh_on_tap_settings_reader(y, y_row3))
        return;
    if (eh_on_tap_settings_folder(y, y_row4))
        return;
    if (eh_on_tap_settings_sysapp(y, y_row5))
        return;
    if (eh_on_tap_settings_apply(y, y_save))
        return;
    if (eh_on_tap_settings_logs(y, y_logs))
        return;

    int y_lic = y_logs + EH_SETTINGS_BTN_H;
    if (eh_on_tap_settings_licenses(y, y_lic))
        return;
}
 ...
 * ignored. */
void
eh_on_tap_log_view(int x, int y)
{
    int bx, by, bw, bh;
    eh_overlay_back_rect(&bx, &by, &bw, &bh);
    if (x >= bx && x < bx + bw && y >= by && y < by + bh) {
        eh_g_state.overlay = EH_OV_NONE;
        eh_g_state.log_scroll = -1;
        eh_redraw_shelf();
        return;
    }
    ...
    if (dir != 0) {
        ...
        int btn_y = h - 8 - EH_SCROLL_BTN_H;
        int page = (btn_y - EH_LOG_BODY_TOP) / EH_LOG_ROW_H;
        if (page < 1)
            page = 1;
        if (eh_g_state.log_scroll < 0) {
            ...
            if (dir < 0) {
                int first = eh_log_view_tail_first() - page;
                eh_g_state.log_scroll = first < 0 ? 0 : first;
            }
        } else {
            /* Rows are ordered oldest → newest; up (dir -1) goes older. */
            eh_g_state.log_scroll += dir * page;
            if (eh_g_state.log_scroll < 0)
                eh_g_state.log_scroll = 0;
        }
        eh_draw_log_view();
...
static int
eh_on_tap_lic_scroll(int x, int y, int btn_y)
{
    int h = eh_content_bottom();
    int dir = eh_hit_scroll_button(x, y);
    if (dir != 0) {
        int body_top = eh_g_state.lic_sel < 0 ? EH_LIC_LIST_TOP : EH_LOG_BODY_TOP;
        int page = eh_g_state.lic_sel < 0
                       ? (btn_y - body_top - 8) / EH_LIC_LIST_H
                       : (btn_y - body_top) / EH_LOG_ROW_H;
        if (page < 1)
            page = 1;
        eh_g_state.lic_scroll += dir * page;
        if (eh_g_state.lic_scroll < 0)
            eh_g_state.lic_scroll = 0;
        eh_draw_licenses_view();
...
static int
eh_on_tap_lic_list(int n, int y, int btn_y)
{
    /* List view: a tap on a row opens that license's full text. */
    if (eh_g_state.lic_sel < 0) {
        int body_h = btn_y - EH_LIC_LIST_TOP - 8;
        int rows_vis = body_h / EH_LIC_LIST_H;
        if (rows_vis < 1)
            rows_vis = 1;
        int rel = (y - EH_LIC_LIST_TOP) / EH_LIC_LIST_H;
        if (y >= EH_LIC_LIST_TOP && rel >= 0 && rel < rows_vis) {
            int idx = eh_g_state.lic_scroll + rel;
            if (idx >= 0 && idx < n) {
                eh_g_state.lic_sel = idx;
                eh_g_state.lic_scroll = 0;
                eh_draw_licenses_view();
                FullUpdate();
            }
...
void
eh_on_tap_licenses_view(int x, int y)
{
    int n = eh_license_count();
    int bx, by, bw, bh;
    eh_overlay_back_rect(&bx, &by, &bw, &bh);
    if (x >= bx && x < bx + bw && y >= by && y < by + bh) {
        if (eh_g_state.lic_sel >= 0) {
    ...
    if (eh_on_tap_lic_scroll(x, y, btn_y))
        return;

    eh_on_tap_lic_list(n, y, btn_y);

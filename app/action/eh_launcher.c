...
{
    if (eh_g_launcher_count >= EH_LAUNCHER_MAX_ITEMS)
        return;
    ...
    if (vis != NULL && !eh_lc_resolve_bool(vis))
        return;
    BsLauncherItem *it = &eh_g_launcher_items[eh_g_launcher_count];
...
{
    for (int i = 0; i < eh_g_launcher_count; i++) {
        if (eh_g_launcher_items[i].kind == 1 && strcmp(eh_g_launcher_items[i].path, path) == 0)
            return 1;
    }
    return 0;
}
...
{
    for (int i = 0; i < eh_g_launcher_count; i++) {
        if (eh_g_launcher_items[i].kind == 0 && (strcmp(eh_g_launcher_items[i].text, "User") == 0 ||
                                              strcmp(eh_g_launcher_items[i].text, "Users") == 0))
            return 1;
    }
    return 0;
}
...
eh_launcher_scan_ext1_apps(void)
{
    ...
    while ((e = readdir(d)) != NULL) {
        ...
        if (launcher_has_path(path))
            continue;
        if (!launcher_has_user_header() && eh_g_launcher_count < EH_LAUNCHER_MAX_ITEMS) {
            BsLauncherItem *hdr = &eh_g_launcher_items[eh_g_launcher_count++];
...
pb_build_groups(const cJSON *groups, const cJSON *db_apps)
{
    ...
    cJSON_ArrayForEach(g, groups) {
        ...
        if (cJSON_IsArray(apps_arr)) {
            if (eh_g_launcher_count < EH_LAUNCHER_MAX_ITEMS && disp_title[0]) {
                BsLauncherItem *hdr = &eh_g_launcher_items[eh_g_launcher_count++];
                memset(hdr, 0, sizeof *hdr);
                hdr->kind = 0;
...
{
    BsLauncherItem *li = &eh_g_launcher_items[eh_g_launcher_count];
...
pb_build_vw_apps(const cJSON *vw_apps)
{
    ...
    cJSON_ArrayForEach(it, vw_apps) {
        ...
        if (!vis)
            continue;
        if (!user_hdr_added && eh_g_launcher_count < EH_LAUNCHER_MAX_ITEMS) {
            BsLauncherItem *hdr = &eh_g_launcher_items[eh_g_launcher_count++];
            memset(hdr, 0, sizeof *hdr);
            hdr->kind = 0;
            snprintf(hdr->text, sizeof hdr->text, "Users");

...
#define PROGRESS_SNAP "/tmp/progress_import.db"
...
#define PROGRESS_COPY_MAX (64 * 1024 * 1024)
...
progress_snapshot(void)
{
    ...
    for (size_t i = 0; i < sizeof suffixes / sizeof suffixes[0]; i++) {
        ...
        snprintf(dst, sizeof dst, "%s%s", PROGRESS_SNAP, suffixes[i]);
        struct stat st, sd;
        if (iv_stat(src, &st) != 0) {
            remove(dst);
            continue;
        ...
        if ((unsigned long long)st.st_size > PROGRESS_COPY_MAX)
            continue;
...
{
    sqlite3 *db = NULL;
    if (sqlite3_open_v2(PROGRESS_SNAP, &db, SQLITE_OPEN_READWRITE, NULL) == SQLITE_OK)
        return db;
